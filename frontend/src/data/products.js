const CDN =
  "https://static.prod-images.emergentagent.com/jobs/ee36022f-d0ea-4536-908f-9c1b130f8012/images";

export const IMG = {
  heroField: "/images/hero-green-farmland.jpg",
  hands: `${CDN}/ad728388dcb701992fd2aa50358fcf1d6f241e13b61cda170cf2e53e95cb8022.jpeg`,
  port: "/images/enquiry-cta-bg.jpg",
  lab: "/images/manifesto.jpg",
  foxnuts: `${CDN}/057bbac088c24df0e3541c1299268e853fc0cdb881519bb8027705acd2f6b6d6.jpeg`,
  millets: "/images/millets/sorghum-jowar.jpg",
  oats: "https://images.unsplash.com/photo-1614961909013-1e2212a2ca8b?q=80&w=1200&auto=format&fit=crop",
  sugar: `${CDN}/b738ca1c6d3335cc6e8c2aa5c6a281d1313cc8f790c318b42bd07684fc42bfcf.jpeg`,
  pulses: "https://images.unsplash.com/photo-1585994192700-112349e5d4cb?q=80&w=1200&auto=format&fit=crop",
  grains: "https://images.unsplash.com/photo-1574323347407-f5e1ad6d020b?q=80&w=1200&auto=format&fit=crop",
  daliya: "https://images.unsplash.com/photo-1607672632458-9eb56696346b?q=80&w=1200&auto=format&fit=crop",
  flours: "https://images.unsplash.com/photo-1556909114-44e8e9c3a26e?q=80&w=1200&auto=format&fit=crop",
};

// ============================================================
// BRAND RANGES
// ============================================================
export const BRAND_RANGES = {
  NATURALS: {
    id: "naturals",
    name: "Harvestgate Naturals",
    tagline: "Ancient grains, pure sweeteners & wholesome nutrition",
    palette: "green", // CSS token: hg-green
    accentColor: "#2E8E50",  // dark mode green
    accentColorLight: "#1C5530", // light mode green
    badge: "HARVESTGATE NATURALS",
    badgeStyle: "green",
  },
  SELECT: {
    id: "select",
    name: "Harvestgate Select",
    tagline: "Premium single-origin specialty exports",
    palette: "gold",
    accentColor: "#F5B923",
    accentColorLight: "#8F6208",
    badge: "HARVESTGATE SELECT",
    badgeStyle: "gold",
  },
};

// ============================================================
// MILLET VARIETIES (10 varieties — unchanged)
// ============================================================
export const MILLET_VARIETIES = [
  {
    id: "pearl-millet",
    name: "Pearl Millet",
    localName: "Bajra",
    botanical: "Pennisetum glaucum",
    image: "/images/millets/sorghum-jowar.jpg",
    hsCode: "1008.21.00",
    origin: "Rajasthan, Gujarat & Haryana",
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.5% Max"],
      ["Damaged / Discolored", "< 1.0% Max"],
      ["Protein", "> 11.6 g / 100g"],
      ["Iron", "> 8.0 mg / 100g"],
    ],
    description: "One of India's most ancient staples, Pearl Millet (Bajra) is rich in iron, zinc, and dietary fiber. Ideal for high-energy flour milling, flatbreads, healthy extruded snacks, and animal feed formulation.",
    highlights: ["High Iron & Zinc", "Energy-Dense Carbohydrates", "Gluten-Free Flour Base", "Rich in Natural Magnesium"],
  },
  {
    id: "finger-millet",
    name: "Finger Millet",
    localName: "Ragi / Mandua",
    botanical: "Eleusine coracana",
    image: "/images/millets/finger-millet-ragi.jpg",
    hsCode: "1008.29.00",
    origin: "Karnataka, Tamil Nadu & Uttarakhand",
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture", "< 10.5% Max"],
      ["Foreign Matter", "< 0.3% Max"],
      ["Calcium", "> 344 mg / 100g (Highest)"],
      ["Dietary Fiber", "> 11.5%"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Finger Millet (Ragi) possesses the highest calcium concentration among all grains (344 mg/100g). Heavily demanded by baby food manufacturers, malt formulations, nutraceuticals, and functional bakery brands.",
    highlights: ["Super-Rich in Calcium", "Natural Antioxidants", "Essential for Infant Nutrition", "Bone-Strengthening Polyphenols"],
  },
  {
    id: "foxtail-millet",
    name: "Foxtail Millet",
    localName: "Kangni / Kakum",
    botanical: "Setaria italica",
    image: "/images/millets/foxtail-kodo.jpg",
    hsCode: "1008.29.00",
    origin: "Andhra Pradesh & Karnataka",
    specs: [
      ["Purity Standard", "100% Natural De-hulled"],
      ["Moisture", "< 9.5% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Protein", "> 12.3 g / 100g"],
      ["Low Glycemic Index", "GI < 50"],
      ["Packaging", "Vacuum / HDPE Woven"],
    ],
    description: "Golden-yellow grains packed with complex carbohydrates and Vitamin B1 (thiamine). Known for its low glycemic index, it is the premier choice for diabetic-friendly food products and ready-to-cook rice alternatives.",
    highlights: ["Low Glycemic Index", "Rich in Vitamin B1", "Direct Rice Substitute", "Supports Steady Blood Sugar"],
  },
  {
    id: "little-millet",
    name: "Little Millet",
    localName: "Kutki / Sama",
    botanical: "Panicum sumatrense",
    image: "/images/millets/little-proso.jpg",
    hsCode: "1008.29.00",
    origin: "Madhya Pradesh & Odisha",
    specs: [
      ["Purity Standard", "100% Natural Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Fat Content", "Healthy PUFA Fats"],
      ["Iron Content", "> 9.3 mg / 100g"],
      ["Cook Time", "10-12 Minutes"],
    ],
    description: "Tiny, delicate ivory grains that cook faster than traditional rice while packing 3x the dietary fiber. Highly versatile for porridge, traditional Indian khichdi, healthy upma, and gluten-free baking blends.",
    highlights: ["Fast Cooking (10 mins)", "High Magnesium & Phosphorus", "Delicate Neutral Flavor", "Rich in Healthy Dietary Fats"],
  },
  {
    id: "kodo-millet",
    name: "Kodon Millet",
    localName: "Kodo / Kodra",
    botanical: "Paspalum scrobiculatum",
    image: "/images/millets/kodo-millet.jpg",
    hsCode: "1008.29.00",
    origin: "Madhya Pradesh, Chhattisgarh & Deccan",
    specs: [
      ["Purity Standard", "100% Natural Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Polyphenols", "Very High Activity"],
      ["Dietary Fiber", "> 9.0%"],
      ["Free from Aflatoxin", "< 4 ppb NABL Tested"],
      ["Packaging", "250g Pouch to 50kg Bags"],
    ],
    description: "Kodo millet contains an extraordinary concentration of polyphenols and lecithin, making it outstanding for nervous system nourishment, cholesterol management, and cellular regeneration.",
    highlights: ["Rich in Polyphenols & Lecithin", "Supports Cardiovascular Health", "Easy Digestibility", "Natural Prebiotic Fiber"],
  },
  {
    id: "sorghum-millet",
    name: "Sorghum Millet",
    localName: "Jowar / Great Millet",
    botanical: "Sorghum bicolor",
    image: "/images/millets/sorghum-jowar.jpg",
    hsCode: "1007.90.00",
    origin: "Maharashtra, Karnataka & Telangana",
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture", "< 11.0% Max"],
      ["Foreign Matter", "< 0.3% Max"],
      ["Color", "Milky White / Light Cream"],
      ["Starch Profile", "Slow Digestible Starch"],
      ["Tannin Free", "Non-GMO Certified"],
    ],
    description: "Large, pearly white grains with a mild sweet flavor. Used globally for gluten-free baking flours, popping, starch manufacturing, craft brewing, and functional extruded snack foods.",
    highlights: ["High Antioxidant Tannins", "Excellent Popping & Flaking Quality", "Clean White Appearance", "Zero-Gluten Baking Champion"],
  },
  {
    id: "proso-millet",
    name: "Proso Millet",
    localName: "Chena / Barri",
    botanical: "Panicum miliaceum",
    image: "/images/millets/proso-millet.jpg",
    hsCode: "1008.29.00",
    origin: "Bihar, UP & Andhra Pradesh",
    specs: [
      ["Purity Standard", "100% Natural Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Protein", "> 12.5% (High Leucine)"],
      ["Minerals", "Rich in Phosphorus & Zinc"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Also known as Broomcorn or Hog millet, Proso boasts the highest protein percentage among millets with a rich profile of essential amino acids like leucine, isoleucine, and methionine.",
    highlights: ["High Complete Protein (12.5%)", "Supports Muscle Synthesis", "Lowest Water Footprint", "Rich in Bioavailable Niacin"],
  },
  {
    id: "browntop-millet",
    name: "Brown Top Millet",
    localName: "Korale / Andu Korralu",
    botanical: "Brachiaria ramosa",
    image: "/images/millets/foxtail-kodo.jpg",
    hsCode: "1008.29.00",
    origin: "Karnataka & Andhra Drylands",
    specs: [
      ["Purity Standard", "100% Natural Unpolished"],
      ["Moisture", "< 9.5% Max"],
      ["Dietary Fiber", "> 12.5% (Highest)"],
      ["Foreign Matter", "< 0.3% Max"],
      ["Gluten Content", "0.0% Naturally Gluten-Free"],
      ["Packaging", "Multi-Wall Paper / HDPE Bags"],
    ],
    description: "Rare and prized, Brown Top millet contains the highest dietary fiber content (12.5%) among all cereal grains. Actively sought after by lifestyle medical centers and detoxification nutrition brands worldwide.",
    highlights: ["Unsurpassed Dietary Fiber (12.5%)", "Natural Gut Detoxifier", "Zero Agricultural Chemicals", "Essential Mineral Fortified"],
  },
  {
    id: "barnyard-millet",
    name: "Barnyard Millet",
    localName: "Sanwa / Jhangora / Samvat",
    botanical: "Echinochloa esculenta",
    image: "/images/millets/barnyard-millet.jpg",
    hsCode: "1008.29.00",
    origin: "Uttarakhand Himalayan Foothills & MP",
    specs: [
      ["Purity Standard", "100% Natural De-husked"],
      ["Moisture", "< 10.0% Max"],
      ["Calorie Content", "Lowest Carb Density"],
      ["Iron Content", "> 18.6 mg / 100g"],
      ["Digestibility", "Fastest Absorbed Grain"],
      ["Certifications", "APEDA, FSSAI, NPOP Available"],
    ],
    description: "The fastest-growing crop in the millet family. Barnyard millet has the lowest carbohydrate density and lowest caloric value, while delivering an astonishing 18.6 mg of bioavailable iron per 100g.",
    highlights: ["Lowest Carbohydrate Density", "Highest Bioavailable Iron", "Himalayan Foothill Provenance", "Fastest Digesting Grain"],
  },
  {
    id: "buckwheat-millet",
    name: "Buckwheat Millet",
    localName: "Kuttu / Buckwheat Groats",
    botanical: "Fagopyrum esculentum",
    image: "/images/millets/buckwheat.jpg",
    hsCode: "1008.10.00",
    origin: "Kashmir Valley, Himachal & Uttarakhand",
    specs: [
      ["Purity Standard", "100% Natural Raw Groats"],
      ["Moisture", "< 11.0% Max"],
      ["Rutin Bioflavonoid", "> 350 mg / 100g"],
      ["Protein", "Complete Protein (9 Amino Acids)"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Formats", "Raw Groats, Roasted Kasha, Fine Flour"],
    ],
    description: "Distinctive triangular dark brown groats containing high concentrations of Rutin, a potent bioflavonoid for cardiovascular health. Provides complete plant protein with all 9 essential amino acids.",
    highlights: ["High Rutin Antioxidant Content", "Complete Plant Protein", "Distinctive Triangular Groat", "Supports Vascular Elasticity"],
  },
];

// ============================================================
// PULSES VARIETIES (10 Certified Export Varieties)
// ============================================================
export const PULSES_VARIETIES = [
  {
    id: "pigeon-pea-split-toor-dal",
    name: "Pigeon Pea Split / Arhar Dal / Toor Dal",
    localName: "Toor Dal / Arhar Dal / Tuvar",
    botanical: "Cajanus cajan",
    image: "/images/pulses/pigeon-pea-split-toor-dal.jpg",
    hsCode: "0713.60.00",
    origin: "Maharashtra, Madhya Pradesh & Karnataka",
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Protein", "> 22.0 g / 100g"],
      ["Finish", "Unpolished / Light Oiled"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Premium yellow split pigeon peas, precision-cleaned and available in natural unpolished and lightly oiled grades. The quintessential Indian staple for aromatic daily dal preparations.",
    highlights: ["Rich in Plant Protein & Potassium", "Quick Cooking & Great Aroma", "Uniform Split & High Nutrient Retention", "High Dietary Fiber"],
  },
  {
    id: "cowpea-white-whole-lobiya",
    name: "Cowpea White Whole / Black Eyebean / Lobiya",
    localName: "Lobiya / Chawli / Black-Eyed Peas",
    botanical: "Vigna unguiculata",
    image: "/images/pulses/cowpea-white-whole-lobiya.jpg",
    hsCode: "0713.35.00",
    origin: "Rajasthan, Gujarat & Maharashtra",
    specs: [
      ["Purity Standard", "100% Natural Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Protein", "> 23.5 g / 100g"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Distinctive creamy-white oval beans characterized by their iconic dark black eye. Renowned globally for quick cooking, savory salads, canned beans, and rich curries.",
    highlights: ["High Soluble Fiber & Folate", "Iconic Black-Eyed Marking", "Global Culinary Versatility", "Low Glycemic Index"],
  },
  {
    id: "green-gram-split-dehusked-moong-dal",
    name: "Green Gram Split Dehusked / Moong Dal Without Chhilka",
    localName: "Moong Dal Dhuli / Yellow Moong Split",
    botanical: "Vigna radiata",
    image: "/images/pulses/green-gram-split-dehusked-moong-dal.jpg",
    hsCode: "0713.31.00",
    origin: "Rajasthan, Maharashtra & Madhya Pradesh",
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Unhusked Seeds", "< 0.2% Max"],
      ["Protein", "> 24.0 g / 100g"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Bright golden-yellow split mung beans, completely dehusked and precision cleaned. Exceptionally light, quick to cook, and the most easily digestible pulse in the world.",
    highlights: ["Easiest to Digest Lentil", "Rich in Antioxidants & Vit B", "Ideal for Khichdi & Soups", "High Bioavailable Iron"],
  },
  {
    id: "green-gram-whole-moong-whole",
    name: "Green Gram Whole / Moong Whole",
    localName: "Sabut Moong / Whole Green Mung",
    botanical: "Vigna radiata",
    image: "/images/pulses/green-gram-whole-moong-whole.jpg",
    hsCode: "0713.31.00",
    origin: "Rajasthan, Karnataka & Andhra Pradesh",
    specs: [
      ["Purity Standard", "100% Natural Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Germination Rate", "> 95% High Sprouting"],
      ["Protein", "> 24.0 g / 100g"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Small, emerald-green whole cylindrical beans with high germination rate. Globally favored for fresh sprouting, nutrient-dense salads, and gluten-free snack formulations.",
    highlights: ["Highest Sprouting Rate (>95%)", "Superfood Micronutrient Density", "Uniform Bold Green Grains", "Plant-Based Protein Core"],
  },
  {
    id: "red-lentil-split-masoor-dal-split",
    name: "Red Lentil Split / Masoor Dal Split",
    localName: "Masoor Dal Dhuli / Malka Split",
    botanical: "Lens culinaris",
    image: "/images/pulses/red-lentil-split-masoor-dal-split.jpg",
    hsCode: "0713.40.00",
    origin: "Madhya Pradesh, Uttar Pradesh & West Bengal",
    specs: [
      ["Purity Standard", "100% Natural Dehusked Split"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.1% Max"],
      ["Protein", "> 26.0 g / 100g"],
      ["Cooking Time", "12-15 Minutes"],
    ],
    description: "Vibrant orange-pink split lentils that cook into a rich, creamy golden soup in under 15 minutes. A staple across Mediterranean, Middle Eastern, and South Asian cuisines.",
    highlights: ["Fast Cooking (12-15 Mins)", "Vibrant Natural Coral Hue", "High Bioavailable Iron", "Creamy Soup Texture"],
  },
  {
    id: "brown-lentil-masoor-whole",
    name: "Brown Lentil / Masoor Whole",
    localName: "Masoor Brown / Sabut Masoor",
    botanical: "Lens culinaris",
    image: "/images/pulses/brown-lentil-masoor-whole.jpg",
    hsCode: "0713.40.00",
    origin: "Madhya Pradesh, Uttar Pradesh & Bihar",
    specs: [
      ["Purity Standard", "100% Natural Cleaned"],
      ["Moisture", "< 10.5% Max"],
      ["Foreign Matter", "< 0.3% Max"],
      ["Protein", "> 24.5 g / 100g"],
      ["Cooking Time", "20-25 Minutes"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Whole brown lentils with a rich earthy flavor and high concentration of dietary protein and soluble fiber. Widely demanded for soups, whole dal preparations, and international retail brands.",
    highlights: ["High Dietary Protein (25%)", "Rich in Folate & Iron", "Natural Earthy Taste", "Nutrient-Dense Daily Dal"],
  },
  {
    id: "bengal-gram-whole-kala-chana",
    name: "Bengal Gram Whole / Chana Whole",
    localName: "Kala Chana / Desi Chana",
    botanical: "Cicer arietinum",
    image: "/images/pulses/bengal-gram-whole-kala-chana.jpg",
    hsCode: "0713.20.00",
    origin: "Madhya Pradesh, Rajasthan & Maharashtra",
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Size Calibration", "7mm / 8mm / 60-70 Count"],
      ["Protein", "> 20.0 g / 100g"],
      ["Packaging", "25kg / 50kg PP Bags"],
    ],
    description: "Dark brown, wrinkled chickpeas renowned for high iron, plant protein, and low glycemic index. Extensively used in traditional curries, sprouting, high-protein snack roasting, and health foods.",
    highlights: ["Very Low Glycemic Index", "Rich in Dietary Fiber & Iron", "High Sprouting Efficiency", "Hearty Robust Taste"],
  },
  {
    id: "black-gram-split-urad-dal-split",
    name: "Black Gram Split / Urad Dal Split",
    localName: "Urad Dal Chilka / Split with Husk",
    botanical: "Vigna mungo",
    image: "/images/pulses/black-gram-split-urad-dal-split.jpg",
    hsCode: "0713.31.00",
    origin: "Andhra Pradesh, Madhya Pradesh & Maharashtra",
    specs: [
      ["Purity Standard", "100% Natural Cleaned"],
      ["Moisture", "< 10.5% Max"],
      ["Broken / Powder", "< 1.0% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Protein", "> 24.0%"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Split black matpe beans retaining the fibrous black husk on one side while exposing the creamy white interior. Delivers robust texture, high iron, and distinct flavor for wholesome lentil soups and dal tadka.",
    highlights: ["High Fiber with Natural Husk", "Rich in Iron & Calcium", "Uniform Split & Natural Protein"],
  },
  {
    id: "black-gram-split-dehusked-urad-white-dal",
    name: "Black Gram Split Dehusked / Urad White Dal",
    localName: "Urad Dal Without Chhilka / Urad Dhuli",
    botanical: "Vigna mungo",
    image: "/images/pulses/black-gram-split-dehusked-urad-white-dal.jpg",
    hsCode: "0713.31.00",
    origin: "Andhra Pradesh, Maharashtra & Uttar Pradesh",
    specs: [
      ["Purity Standard", "100% Natural Dehusked"],
      ["Moisture", "< 10.0% Max"],
      ["Unhusked Seeds", "< 0.2% Max"],
      ["Protein", "> 25.0 g / 100g"],
      ["Finish", "Natural Unpolished / Water-Polished"],
      ["Packaging", "Multi-wall Paper & PP Bags"],
    ],
    description: "Creamy ivory-white split beans, completely free of seed coat. Revered worldwide for its extraordinary foaming, fermentation, and binding properties essential for papads, south-Indian dosas/idlis, and vada batters.",
    highlights: ["Superior Fermentation Profile", "Ultra-Clean Ivory Hue", "Papad & Batter Standard"],
  },
  {
    id: "black-gram-whole-urad-whole",
    name: "Black Gram Whole / Urad Whole",
    localName: "Sabut Urad / Black Matpe",
    botanical: "Vigna mungo",
    image: "/images/pulses/black-gram-whole-urad-whole.jpg",
    hsCode: "0713.31.00",
    origin: "Andhra Pradesh, Maharashtra & Madhya Pradesh",
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Size Grading", "Uniform Bold Grain"],
      ["Protein", "> 24.0%"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Small, matte-black cylindrical whole beans with high mucilage and starch content. The foundational ingredient of world-famous Dal Makhani and ethnic canned meal formulations across the globe.",
    highlights: ["Dal Makhani Essential", "High Natural Mucilage", "Even Size & Deep Black Lustre"],
  },
  {
    id: "soybean-whole",
    name: "Soybean Whole",
    localName: "Bhat / Whole Soybeans",
    botanical: "Glycine max",
    image: "/images/pulses/soybean-whole.jpg",
    hsCode: "1201.90.00",
    origin: "Madhya Pradesh, Maharashtra & Rajasthan",
    specs: [
      ["Purity Standard", "100% Natural Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.3% Max"],
      ["Protein", "> 38.0 g / 100g"],
      ["Oil Content", "> 18.0%"],
      ["Shelf Life", "24 Months"],
    ],
    description: "High-protein, non-GMO whole yellow soybeans cleaned and size-graded. Ideal for soy milk extraction, tofu, textured vegetable protein (TVP), flour blending, and food processing.",
    highlights: ["Ultra-High Protein (>38%)", "Non-GMO & Export Graded", "Complete Amino Acid Profile"],
  },
  {
    id: "red-lentil-whole-masoor-dal",
    name: "Red Lentil Whole / Masoor Dal",
    localName: "Sabut Lal Masoor / Football Masoor",
    botanical: "Lens culinaris",
    image: "/images/pulses/red-lentil-whole-masoor-dal.jpg",
    hsCode: "0713.40.00",
    origin: "Madhya Pradesh, Uttar Pradesh & West Bengal",
    specs: [
      ["Purity Standard", "100% Natural Dehusked Whole"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Protein", "> 25.5 g / 100g"],
      ["Calibration", "Football Type Bold Round"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Whole round red lentils (football type) featuring deep coral-red color and quick cooking time. A major global export commodity highly demanded across the Middle East, North Africa, and Europe.",
    highlights: ["Rich Coral-Red Hue", "High Dietary Fiber & Iron", "Uniform Bold Calibration", "Nutrient-Dense Mineral Core"],
  },
  {
    id: "chickpea-kabuli-chana",
    name: "Chickpea / Kabuli Chana",
    localName: "Kabuli Chana / White Chickpeas / Garbanzo",
    botanical: "Cicer arietinum",
    image: "/images/pulses/chickpea-kabuli-chana.jpg",
    hsCode: "0713.20.00",
    origin: "Madhya Pradesh, Maharashtra & Rajasthan",
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Calibration / Count", "75-80 / 58-60 / 42-44 Count (Bold)"],
      ["Protein", "> 20.5 g / 100g"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Large, creamy-beige bold chickpeas with distinctive wrinkled skin and nutty flavour. Highly prized globally for hummus manufacturing, canning, Mediterranean salads, and traditional gourmet curries.",
    highlights: ["Extra Bold Export Calibration", "High Plant Protein & Dietary Fiber", "Nutty Gourmet Texture", "Ideal for Hummus & Canning"],
  },
  {
    id: "bengal-gram-split-chana-dal",
    name: "Bengal Gram Split / Chana Dal",
    localName: "Chana Dal / Split Desi Chickpeas",
    botanical: "Cicer arietinum",
    image: "/images/pulses/bengal-gram-split-chana-dal.jpg",
    hsCode: "0713.20.00",
    origin: "Madhya Pradesh, Rajasthan & Maharashtra",
    specs: [
      ["Purity Standard", "100% Natural Dehusked Split"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.15% Max"],
      ["Protein", "> 21.0 g / 100g"],
      ["Finish", "Natural Unpolished"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Golden-yellow split chickpeas derived from carefully hulled desi chana. Renowned for its rich sweet-nutty taste, firm cooking texture, and high zinc and protein content. The primary ingredient for premium besan flour and hearty dal curries.",
    highlights: ["Exceptionally Low Glycemic Index", "Rich in Natural Plant Zinc & Fiber", "Firm Texture & Nutty Flavor", "Essential for Besan & Dal Tadka"],
  },
  {
    id: "green-gram-split-moong-dal-chhilka",
    name: "Green Gram Split / Moong Dal Chhilka",
    localName: "Moong Dal Chilka / Split Green Mung with Husk",
    botanical: "Vigna radiata",
    image: "/images/pulses/green-gram-split-moong-dal-chhilka.jpg",
    hsCode: "0713.31.00",
    origin: "Rajasthan, Maharashtra & Madhya Pradesh",
    specs: [
      ["Purity Standard", "100% Natural Cleaned Split"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Protein", "> 24.0 g / 100g"],
      ["Dietary Fiber", "> 16.0%"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Split green mung beans retaining their nutrient-rich green skin on the exterior while revealing tender golden cotyledons. Offers the perfect balance of gentle digestibility and high dietary fiber.",
    highlights: ["Intact Fiber-Rich Outer Husk", "Gentle & Fast Digesting", "High Potassium & Magnesium", "Wholesome Daily Khichdi Dal"],
  },
  {
    id: "cowpea-red-whole-red-lobiya",
    name: "Cowpea Red Whole / Red Lobiya",
    localName: "Lal Lobiya / Red Chawli / Adzuki-Type Cowpea",
    botanical: "Vigna unguiculata",
    image: "/images/pulses/cowpea-red-whole-red-lobiya.jpg",
    hsCode: "0713.35.00",
    origin: "Maharashtra, Karnataka & Tamil Nadu",
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Protein", "> 23.0 g / 100g"],
      ["Color Calibration", "Deep Crimson Red Lustre"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Glossy deep-crimson whole cowpeas possessing an earthy aroma and firm bite when cooked. Highly demanded in ethnic retail, bean salads, stews, and vegetarian protein blends across global markets.",
    highlights: ["Rich Anthocyanin & Iron Content", "Firm Cooking Texture", "Dense Plant Protein Core", "High Dietary Soluble Fiber"],
  },
  {
    id: "kidney-bean-chitra-rajma-chitra",
    name: "Kidney Bean Chitra / Rajma Chitra",
    localName: "Chitra Rajma / Speckled Kashmiri Kidney Beans",
    botanical: "Phaseolus vulgaris",
    image: "/images/pulses/kidney-bean-chitra-rajma-chitra.jpg",
    hsCode: "0713.33.00",
    origin: "Himalayan Belts of Jammu & Kashmir, Himachal & Uttarakhand",
    specs: [
      ["Purity Standard", "100% Natural Cleaned"],
      ["Moisture", "< 10.5% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Protein", "> 22.5 g / 100g"],
      ["Pattern", "Speckled Crimson Streaks on Cream"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Celebrated speckled kidney beans with beige skin decorated with intricate reddish-brown streaks. Sourced from high-altitude Himalayan valleys, renowned for meltingly soft, buttery texture and sweet aromatic gravy.",
    highlights: ["Himalayan Valley Provenance", "Melt-in-Mouth Buttery Texture", "Iconic Speckled Chitra Markings", "Rich in Folate & Complex Carbs"],
  },
  {
    id: "kidney-bean-jammu-rajma-jammu",
    name: "Kidney Bean Jammu / Rajma Jammu",
    localName: "Bhaderwah Rajma / Kashmiri Small Red Rajma",
    botanical: "Phaseolus vulgaris",
    image: "/images/pulses/kidney-bean-jammu-rajma-jammu.jpg",
    hsCode: "0713.33.00",
    origin: "Chenab Valley & Bhaderwah (Jammu & Kashmir)",
    specs: [
      ["Purity Standard", "100% Natural Himalayan Sourced"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.15% Max"],
      ["Size Calibration", "Small Compact Gourmet Grain"],
      ["Protein", "> 23.5 g / 100g"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Small, lustrous ruby-red beans grown exclusively in the high-altitude microclimates of Jammu & Kashmir. Celebrated across India as the finest culinary rajma for its naturally sweet taste, melting texture, and distinctive rich aroma.",
    highlights: ["GI-Origin Bhaderwah Microclimate", "Naturally Sweet & Intense Aroma", "Meltingly Soft Texture", "Celebrated North Indian Gourmet Standard"],
  },
  {
    id: "red-kidney-bean-rajma-red",
    name: "Red Kidney Bean / Rajma Red",
    localName: "Lal Rajma / Dark Red Kidney Beans / Sharmili",
    botanical: "Phaseolus vulgaris",
    image: "/images/pulses/red-kidney-bean-rajma-red.jpg",
    hsCode: "0713.33.00",
    origin: "Maharashtra, Madhya Pradesh & Uttarakhand",
    specs: [
      ["Purity Standard", "100% Natural Cleaned"],
      ["Moisture", "< 10.5% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Calibration / Count", "180-200 / 220-240 per 100g"],
      ["Protein", "> 24.0 g / 100g"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Classic deep mahogany red kidney beans with firm curved shape and rich earthy flavor. Highly valued for hearty curries, chili con carne, canned bean manufacturing, and Latin American/European stews.",
    highlights: ["Classic Large Kidney Calibration", "High Plant Protein & Dietary Fiber", "Firm Shape Retention After Cooking", "Rich Mahogany Red Color"],
  },
];

// ============================================================
// GRAINS VARIETIES (Certified Indian Grains & Cereals)
// ============================================================
export const GRAINS_VARIETIES = [
  {
    id: "maize-corn-makka",
    name: "Maize / Corn / Makka",
    localName: "Yellow Maize / Makka",
    botanical: "Zea mays",
    image: "/images/grains/maize-corn-makka.jpg",
    hsCode: "1005.90.00",
    origin: "Madhya Pradesh, Karnataka & Bihar",
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture", "< 12.0% Max"],
      ["Aflatoxin", "< 20 ppb Certified"],
      ["Foreign Matter", "< 1.0% Max"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Bright yellow, plump, precision-cleaned yellow dent maize with low moisture. Premium export grade for food processing, starch extraction, cornflour, and poultry/animal nutrition.",
    highlights: ["High Starch & Energy Profile", "Aflatoxin < 20 ppb Certified", "Uniform Bold Golden Kernels", "High Natural Carotenoids"],
  },
  {
    id: "amaranth-rajgira",
    name: "Amaranth / Rajgira",
    localName: "Rajgira / Ramdana / Chaulai",
    botanical: "Amaranthus cruentus",
    image: "/images/grains/amaranth-rajgira.jpg",
    hsCode: "1008.90.00",
    origin: "Gujarat, Maharashtra & Uttarakhand",
    specs: [
      ["Purity Standard", "100% Natural Cleaned"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Protein", "> 14.5 g / 100g"],
      ["Calcium", "> 159 mg / 100g"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Tiny, ivory-golden ancient supergrain loaded with complete plant protein, squalene, and calcium. Highly sought-after for gluten-free cereal, energy bars, popping, and healthy baking blends.",
    highlights: ["Complete Protein with Lysine", "Rich in Squalene & Calcium", "Naturally Gluten-Free", "Ancient Sacred Supercrop"],
  },
  {
    id: "barley-jau",
    name: "Barley / Jau",
    localName: "Jau / Hulled & Pearl Barley",
    botanical: "Hordeum vulgare",
    image: "/images/grains/barley-jau.jpg",
    hsCode: "1003.90.00",
    origin: "Rajasthan, Uttar Pradesh & Haryana",
    specs: [
      ["Purity Standard", "100% Natural Cleaned"],
      ["Moisture", "< 11.0% Max"],
      ["Foreign Matter", "< 0.5% Max"],
      ["Beta-Glucan", "> 4.5%"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Fiber-rich, clean, premium golden barley grains with exceptional soluble beta-glucan content. Extensively used in craft malting, brewing, breakfast porridge, and functional nutrition.",
    highlights: ["High Beta-Glucan Soluble Fiber", "Malting & Brewing Grade", "Supports Blood Sugar Balance", "Heart Health Supergrain"],
  },
  {
    id: "1121-xxl-basmati-rice",
    name: "1121 XXL Basmati Rice",
    localName: "1121 Extra Long Grain Basmati",
    botanical: "Oryza sativa",
    image: "https://images.unsplash.com/photo-1586201375761-83865001e31c?q=80&w=1200&auto=format&fit=crop",
    hsCode: "1006.30.20",
    origin: "Punjab, Haryana & Western UP",
    specs: [
      ["Average Grain Length", "8.35 mm+ (Aged 2 Yrs)"],
      ["Purity Standard", "100% Natural Silky Finish"],
      ["Moisture", "< 12.0% Max"],
      ["Broken Grains", "< 1.0% Max"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Aged for 2 years to develop unmatched cooking elongation (up to 22mm), vintage floral aroma, and delicate fluffy texture for luxury catering and global retail.",
    highlights: ["8.35mm+ Raw Grain Length", "2x Cooking Elongation", "Aged for Vintage Fragrance"],
  },
  {
    id: "sharbati-wheat",
    name: "Sharbati Milling Wheat",
    localName: "Sharbati Gehu / Golden Wheat",
    botanical: "Triticum aestivum",
    image: "https://images.unsplash.com/photo-1574323347407-f5e1ad6d020b?q=80&w=1200&auto=format&fit=crop",
    hsCode: "1001.99.10",
    origin: "Sehore & Vidisha, Madhya Pradesh",
    specs: [
      ["Purity", "99.5% Machine Cleaned"],
      ["Moisture", "< 11.5% Max"],
      ["Protein", "> 12.5%"],
      ["Gluten Index", "High Gluten Strength"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Heavy golden grains harvested from the black fertile soils of Madhya Pradesh. Celebrated as the gold standard of wheat for softest rotis, flatbreads, and premium semolina.",
    highlights: ["High Protein (>12.5%)", "Golden Lustre & Heavy Grains", "Premium Flatbread Atta Base"],
  },
  {
    id: "wheat-whole",
    name: "Wheat Whole",
    localName: "Sabut Gehu / Whole Milling Wheat Berries",
    botanical: "Triticum aestivum",
    image: "/images/grains/wheat-whole.jpg",
    hsCode: "1001.99.10",
    origin: "Madhya Pradesh, Punjab & Haryana",
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture", "< 11.0% Max"],
      ["Foreign Matter", "< 0.3% Max"],
      ["Protein", "> 12.5% - 13.5%"],
      ["Hectolitre Weight", "> 78 - 80 kg/hL"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Plump, golden, hard-grained whole wheat berries selected for high test weight and superior gluten strength. Ideal for wholemeal flour milling, traditional stone chakki atta, breakfast grain bowls, and commercial bakery formulations.",
    highlights: ["High Test Weight & Dense Berries", "High Natural Protein (>13%)", "Superior Gluten Elasticity", "Stone-Ground Atta Standard"],
  },
  {
    id: "quinoa",
    name: "Quinoa",
    localName: "White Indian Quinoa / Kinwa",
    botanical: "Chenopodium quinoa",
    image: "/images/grains/quinoa.jpg",
    hsCode: "1008.50.00",
    origin: "Rajasthan & Andhra Pradesh",
    specs: [
      ["Purity Standard", "100% Saponin-Free Washed"],
      ["Moisture", "< 10.0% Max"],
      ["Foreign Matter", "< 0.1% Max"],
      ["Protein", "> 14.0 g / 100g (Complete)"],
      ["Gluten Content", "0.0% Naturally Gluten-Free"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Saponin-washed, premium ivory-white quinoa seeds cultivated in India's arid belts. A celebrated pseudocereal offering a complete amino acid profile, high dietary fiber, and a light, fluffy, slightly crunchy texture when cooked.",
    highlights: ["Complete Amino Acid Profile", "100% Saponin-Free & Washed", "Naturally Gluten-Free Superseed", "Light Fluffy Cooking Texture"],
  },
  {
    id: "oats-dehulled",
    name: "Oats Dehulled",
    localName: "Dehulled Whole Oat Groats / Sabut Jai",
    botanical: "Avena sativa",
    image: "/images/grains/oats-dehulled.jpg",
    hsCode: "1104.22.00",
    origin: "Rajasthan, Haryana & Punjab",
    specs: [
      ["Purity Standard", "100% Machine Cleaned & Dehulled Groats"],
      ["Moisture", "< 10.5% Max"],
      ["Foreign Matter", "< 0.2% Max"],
      ["Beta-Glucan", "> 4.5 g / 100g"],
      ["Protein", "> 13.0% Min"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Sound, plump whole oat groats with the fibrous outer hull cleanly removed, retaining the intact nutrient-dense germ, bran layer, and endosperm. Rich in soluble dietary fiber (beta-glucan), plant protein, and essential minerals for industrial flaking, milling, and health food formulations.",
    highlights: ["Whole Grain Intact Germ & Bran", "High Soluble Beta-Glucan Fiber", "100% Dehulled Cleaned Groats", "Ideal for Flaking & Milling"],
  },
  {
    id: "oats-rolled",
    name: "Oats Rolled",
    localName: "Rolled Oat Flakes / Jai Ke Phulke",
    botanical: "Avena sativa",
    image: "/images/grains/oats-rolled.jpg",
    hsCode: "1104.12.00",
    origin: "Rajasthan, Haryana & Punjab",
    specs: [
      ["Purity Standard", "100% Steamed & Rolled Whole Oats"],
      ["Moisture", "< 10.0% Max"],
      ["Beta-Glucan", "> 4.2 g / 100g"],
      ["Flake Thickness", "0.5 - 0.7 mm Uniform Rolled"],
      ["Dietary Fiber", "> 10.5%"],
      ["Shelf Life", "18 Months"],
    ],
    description: "Whole de-hulled oat groats gently steamed and kiln-rolled into uniform large flakes. Rich in heart-healthy soluble beta-glucan fiber and plant protein, delivering sustained satiety for breakfast cereals, granola, muesli, and bakery blends.",
    highlights: ["Rich in Soluble Beta-Glucan", "Steamed & Kiln-Rolled Flakes", "Cardiovascular Wellness Standard", "Ideal for Muesli & Granola"],
  },
  {
    id: "white-poha-beaten-rice",
    name: "White Poha / Beaten Rice White",
    localName: "White Poha / Chivda / Aval / Flattened Rice",
    botanical: "Oryza sativa",
    image: "/images/grains/white-poha-beaten-rice.jpg",
    hsCode: "1104.19.00",
    origin: "Madhya Pradesh (Ujjain), Maharashtra & Gujarat",
    specs: [
      ["Purity Standard", "100% Natural De-husked & Flattened"],
      ["Moisture", "< 11.5% Max"],
      ["Broken Flakes", "< 2.0% Max"],
      ["Flake Calibration", "Medium / Thick Export Grade"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Traditional Indian flattened rice flakes prepared by parboiling, rolling, and flattening cleaned paddy rice. Exceptionally light, easily digestible, and quick to rehydrate, making it India's beloved wholesome breakfast and savory snack staple.",
    highlights: ["Light & Instant Digesting", "Low Fat & High Iron Carrier", "Uniform Medium-Thick Flakes", "Quintessential Indian Breakfast"],
  },
];

// ============================================================
// FLOURS VARIETIES (Certified Indian Flours & Atta)
// ============================================================
export const FLOURS_VARIETIES = [
  {
    id: "amaranth-rajgira-flour",
    name: "Amaranth / Rajgira Flour",
    localName: "Rajgira Atta / Ramdana Flour",
    botanical: "Amaranthus cruentus",
    image: "/images/flours/amaranth-rajgira-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Gujarat, Maharashtra & Uttarakhand",
    specs: [
      ["Purity", "100% Pure Stone-Ground"],
      ["Moisture", "< 10.0% Max"],
      ["Protein", "> 14.0 g / 100g"],
      ["Gluten Content", "0.0% Naturally Gluten-Free"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Ultra-fine ivory flour stone-ground from cleaned raw amaranth seeds. Packed with complete plant protein, squalene, and bioavailable calcium. Ideal for gluten-free flatbreads, fasting dishes, and health bakery blends.",
    highlights: ["Complete Protein with Lysine", "Rich in Bioavailable Calcium", "Fine Stone-Ground Texture"],
  },
  {
    id: "besan-bengal-gram-flour",
    name: "Besan / Bengal Gram Flour",
    localName: "Pure Chana Besan / Chickpea Flour",
    botanical: "Cicer arietinum",
    image: "/images/flours/besan-bengal-gram-flour.jpg",
    hsCode: "1106.10.00",
    origin: "Madhya Pradesh & Rajasthan",
    specs: [
      ["Purity", "100% Pure Chana Dal"],
      ["Moisture", "< 10.5% Max"],
      ["Protein", "> 20.5 g / 100g"],
      ["Texture", "Superfine Silky Sifted"],
      ["Shelf Life", "12 Months"],
    ],
    description: "100% pure de-husked Bengal gram (chana dal) ground to silky perfection with zero adulteration or additives. The gold standard for batter frying, extruded savories, and traditional confectionery.",
    highlights: ["100% Pure Chana Dal Base", "High Plant Protein (>20%)", "Silky Smooth Fine Texture"],
  },
  {
    id: "barnyard-millet-flour",
    name: "Barnyard Millet Flour",
    localName: "Sanwa Ka Atta / Samvat Flour",
    botanical: "Echinochloa esculenta",
    image: "/images/flours/barnyard-millet-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Uttarakhand Himalayan Foothills & MP",
    specs: [
      ["Purity", "100% Single-Origin Millet"],
      ["Moisture", "< 10.0% Max"],
      ["Iron Content", "High Bioavailable Iron"],
      ["Carb Density", "Lowest Calorie Cereal Flour"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Lightweight, easy-to-digest flour milled from Himalayan foothills barnyard millet. Features the lowest carbohydrate density and highest bioavailable iron content among cereal flours.",
    highlights: ["Lowest Calorie & Carb Density", "Highest Bioavailable Iron", "Fast Digestion Profile"],
  },
  {
    id: "buckwheat-kuttu-flour",
    name: "Buckwheat / Kuttu Flour",
    localName: "Kuttu Ka Atta / Buckwheat Flour",
    botanical: "Fagopyrum esculentum",
    image: "/images/flours/buckwheat-kuttu-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Kashmir Valley, Himachal & Uttarakhand",
    specs: [
      ["Purity", "100% Whole Buckwheat Groats"],
      ["Moisture", "< 11.0% Max"],
      ["Rutin Bioflavonoid", "> 350 mg / 100g"],
      ["Gluten Content", "0.0% Naturally Gluten-Free"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Milled from whole triangular buckwheat groats into a characteristically speckled, nutty flour rich in Rutin bioflavonoids. Widely demanded globally for soba noodles, crepes, and gluten-free galettes.",
    highlights: ["High Rutin Antioxidant Content", "Rich Nutty Flavor Profile", "Ideal for Soba & Galettes"],
  },
  {
    id: "foxtail-millet-flour",
    name: "Foxtail Millet Flour",
    localName: "Kangni Ka Atta / Kakum Flour",
    botanical: "Setaria italica",
    image: "/images/flours/foxtail-millet-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Andhra Pradesh & Karnataka",
    specs: [
      ["Purity", "100% Unpolished Foxtail Millet"],
      ["Moisture", "< 10.0% Max"],
      ["Glycemic Index", "Low GI (< 50)"],
      ["Dietary Fiber", "> 8.0%"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Finely milled from sorted, de-hulled foxtail millet grains. Delivers a low glycemic index and high dietary fiber, making it the premier choice for diabetic-friendly gluten-free flatbreads and healthy baking.",
    highlights: ["Low Glycemic Index (GI < 50)", "High Vitamin B1 & Dietary Fiber", "100% Gluten-Free Baking Base"],
  },
  {
    id: "oats-flour",
    name: "Oats Flour",
    localName: "Jayi Ka Atta / Whole Oat Flour",
    botanical: "Avena sativa",
    image: "/images/flours/oats-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Punjab, Haryana & Rajasthan",
    specs: [
      ["Purity", "100% Whole Grain Rolled Oats"],
      ["Moisture", "< 10.5% Max"],
      ["Beta-Glucan", "> 4.0%"],
      ["Protein", "> 13.0 g / 100g"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Finely ground from 100% whole grain rolled oats. Rich in cholesterol-lowering soluble beta-glucan fiber, vitamins, and minerals. Perfect for oat bread, pancakes, cookies, and baby food formulas.",
    highlights: ["High Soluble Beta-Glucan", "Supports Heart Health", "Smooth Fine Texture"],
  },
  {
    id: "kodo-millet-flour",
    name: "Kodon Millet / Kodo Flour",
    localName: "Kodon Ka Atta / Varagu Flour",
    botanical: "Paspalum scrobiculatum",
    image: "/images/flours/kodo-millet-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Madhya Pradesh, Chhattisgarh & Tamil Nadu",
    specs: [
      ["Purity", "100% Unpolished Kodo Millet"],
      ["Moisture", "< 10.0% Max"],
      ["Polyphenol Content", "Extremely High Antioxidant"],
      ["Dietary Fiber", "> 9.0%"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Milled from whole unpolished kodo millet grains containing exceptional polyphenols and antioxidants. Famous for diabetic wellness, weight management, and gluten-free traditional bakery.",
    highlights: ["Ultra-High Polyphenol Content", "Anti-Diabetic Properties", "100% Gluten-Free"],
  },
  {
    id: "maida-refined-wheat-flour",
    name: "Maida / Refined Wheat Flour",
    localName: "Fine Maida / All-Purpose Flour",
    botanical: "Triticum aestivum",
    image: "/images/flours/maida-refined-wheat-flour.jpg",
    hsCode: "1101.00.00",
    origin: "Punjab, Madhya Pradesh & Uttar Pradesh",
    specs: [
      ["Extraction Rate", "70-72% Superfine"],
      ["Moisture", "< 12.0% Max"],
      ["Wet Gluten", "> 28.0%"],
      ["Ash Content", "< 0.55% Max"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Superfine, silky, roller-milled wheat flour with calibrated wet gluten strength and high water absorption. Premium commercial standard for bakery products, pastries, biscuits, and confectionery.",
    highlights: ["Calibrated Wet Gluten Strength", "Silky Superfine Particle Size", "Bakery & Confectionery Grade"],
  },
  {
    id: "maize-corn-makka-flour",
    name: "Maize / Corn / Makka Flour",
    localName: "Makki Ka Atta / Yellow Corn Flour",
    botanical: "Zea mays",
    image: "/images/flours/maize-corn-makka-flour.jpg",
    hsCode: "1102.20.00",
    origin: "Punjab, Madhya Pradesh & Karnataka",
    specs: [
      ["Purity", "100% Yellow Maize"],
      ["Moisture", "< 11.0% Max"],
      ["Carotenoids", "High Natural Pigment"],
      ["Texture", "Stone-Ground Medium Fine"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Bright yellow, stone-ground flour milled from clean, selected yellow corn. Imparts golden color, earthy aroma, and authentic crispness to tortillas, makki ki roti, corn breads, and snack batters.",
    highlights: ["Vibrant Golden Corn Hue", "Rich in Carotenoids & Fiber", "Authentic Makki Roti & Tortilla Base"],
  },
  {
    id: "multigrain-flour-atta",
    name: "Multi Grain Flour / Multi Grain Atta Flour",
    localName: "Navratna Multigrain Atta",
    botanical: "Triticum + Millets + Pulses Blend",
    image: "/images/flours/multigrain-flour-atta.jpg",
    hsCode: "1101.00.00",
    origin: "Madhya Pradesh, Rajasthan & Uttar Pradesh",
    specs: [
      ["Composition", "Wheat + Ragi + Jowar + Chana + Soya + Oats"],
      ["Moisture", "< 11.5% Max"],
      ["Protein", "> 13.5%"],
      ["Dietary Fiber", "> 10.5%"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Expertly balanced blend of Sharbati wheat, whole ragi, jowar, bajra, chana dal, soybean, and oats. Provides a complete amino acid profile, high dietary fiber, and superior roti fluffiness.",
    highlights: ["9-Grain Nutritional Balance", "High Protein & Soluble Fiber", "Soft, Fluffy & Wholesome Rotis"],
  },
  {
    id: "whole-wheat-chakki-atta",
    name: "Whole Wheat Flour / Wheat Chakki Atta Flour",
    localName: "Sharbati Chakki Fresh Atta",
    botanical: "Triticum aestivum",
    image: "/images/flours/whole-wheat-chakki-atta.jpg",
    hsCode: "1101.00.00",
    origin: "Madhya Pradesh & Punjab",
    specs: [
      ["Purity", "100% Whole Wheat Single-Origin"],
      ["Moisture", "< 11.5% Max"],
      ["Extraction Rate", "100% Whole Meal with Bran & Germ"],
      ["Protein", "> 12.5 g / 100g"],
      ["Shelf Life", "12 Months"],
    ],
    description: "100% whole grain wheat slow-ground in traditional stone chakkis to retain all bran, germ, and natural endosperm nutrients. Produces extra soft, aromatic, and nutrient-dense rotis.",
    highlights: ["100% Whole Grain Stone-Ground", "High Fiber with Wheat Germ", "Extra Soft Roti Texture"],
  },
  {
    id: "pearl-millet-bajra-flour",
    name: "Pearl Millet / Bajra Flour",
    localName: "Bajre Ka Atta / Pearl Millet Flour",
    botanical: "Pennisetum glaucum",
    image: "/images/flours/pearl-millet-bajra-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Rajasthan, Gujarat & Haryana",
    specs: [
      ["Purity", "100% Pure Unpolished Bajra"],
      ["Moisture", "< 10.0% Max"],
      ["Iron Content", "High Bioavailable Iron & Zinc"],
      ["Dietary Fiber", "> 11.0%"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Mineral-rich, rustic flour milled from premium desert pearl millet. Exceptional source of plant iron, magnesium, and phosphorus, widely eaten during winter for warming stamina.",
    highlights: ["High Bioavailable Iron & Zinc", "Natural Warming Stamina Base", "Rich in Complex Fibers"],
  },
  {
    id: "quinoa-flour",
    name: "Quinoa Flour",
    localName: "White Quinoa Atta",
    botanical: "Chenopodium quinoa",
    image: "/images/flours/quinoa-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Andhra Pradesh & Rajasthan",
    specs: [
      ["Purity", "100% Saponin-Free Quinoa"],
      ["Moisture", "< 10.0% Max"],
      ["Protein", "> 14.5 g / 100g (All 9 Amino Acids)"],
      ["Gluten Content", "0.0% Naturally Gluten-Free"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Silky, stone-milled flour from washed, saponin-free white quinoa seeds. Delivers complete plant protein with all nine essential amino acids and an ultra-low glycemic response.",
    highlights: ["Complete 9 Amino Acids", "Saponin-Free Wash Certified", "Gluten-Free Superfood Base"],
  },
  {
    id: "semolina-suji-flour",
    name: "Semolina / Suji Flour",
    localName: "Rawa / Durum Wheat Semolina",
    botanical: "Triticum durum",
    image: "/images/flours/semolina-suji-flour.jpg",
    hsCode: "1103.11.00",
    origin: "Madhya Pradesh & Maharashtra",
    specs: [
      ["Purity", "100% Hard Durum Wheat"],
      ["Moisture", "< 12.0% Max"],
      ["Granulation", "Medium Coarse Uniform Grit"],
      ["Protein", "> 12.0 g / 100g"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Coarsely purified durum wheat semolina with uniform golden granules. Ideal for pasta, couscous, savory upma, halwa desserts, and crisp bakery crusts.",
    highlights: ["100% Durum Wheat Base", "Uniform Granular Calibration", "Crisp Texture & High Stability"],
  },
  {
    id: "sorghum-millet-jowar-flour",
    name: "Sorghum Millet / Jowar Flour",
    localName: "Jowar Atta / White Sorghum Flour",
    botanical: "Sorghum bicolor",
    image: "/images/flours/sorghum-millet-jowar-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Maharashtra & Karnataka",
    specs: [
      ["Purity", "100% Maldandi Shalu Jowar"],
      ["Moisture", "< 10.5% Max"],
      ["Resistant Starch", "High Fiber Profile"],
      ["Gluten Content", "0.0% Naturally Gluten-Free"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Mild, creamy white flour milled from selected Maldandi sorghum grains. Naturally gluten-free with high resistant starch, ideal for bhakri flatbreads, gluten-free baking mixes, and cookies.",
    highlights: ["Maldandi Shalu Origin", "High Resistant Starch", "Mild Nutty Taste for Bhakri"],
  },
  {
    id: "little-millet-flour",
    name: "Little Millet Flour",
    localName: "Kutki Ka Atta / Samai Flour",
    botanical: "Panicum sumatrense",
    image: "/images/flours/little-millet-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Madhya Pradesh, Tamil Nadu & Karnataka",
    specs: [
      ["Purity", "100% Unpolished Little Millet"],
      ["Moisture", "< 10.0% Max"],
      ["Iron Content", "> 9.3 mg / 100g"],
      ["PUFA Healthy Fats", "Rich Essential Fatty Acids"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Fine ivory-cream flour stone-milled from unpolished, de-hulled little millet grains. Packed with heart-healthy polyunsaturated fatty acids (PUFA), bioavailable plant iron, and dietary fiber. Ideal for diabetic-friendly flatbreads, fasting rotis, and wholesome porridge mixes.",
    highlights: ["Rich in Essential PUFA Fats", "High Plant Iron & Antioxidants", "100% Gluten-Free Superfood Base"],
  },
  {
    id: "rice-chawal-flour",
    name: "Rice / Chawal Flour",
    localName: "Chawal Ka Atta / Akki Hittu",
    botanical: "Oryza sativa",
    image: "/images/flours/rice-chawal-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Punjab, Haryana, Chhattisgarh & West Bengal",
    specs: [
      ["Purity", "100% Pure White Rice Base"],
      ["Moisture", "< 11.0% Max"],
      ["Fineness", "Superfine Silky Milled (< 100 Mesh)"],
      ["Gluten Content", "0.0% Naturally Gluten-Free"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Silky, ultra-fine white flour stone-milled from premium selected non-sticky Indian rice grains. Naturally gluten-free, easily digestible, and prized worldwide for authentic rice noodles, crispy dosas, idlis, gluten-free bakery dusting, and batters.",
    highlights: ["100% Naturally Gluten-Free", "Superfine Silky Texture", "Ideal for Crispy Batters & Idli / Dosa"],
  },
  {
    id: "barley-jau-flour",
    name: "Barley / Jau Flour",
    localName: "Jau Ka Atta / Whole Barley Flour",
    botanical: "Hordeum vulgare",
    image: "/images/flours/barley-jau-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Rajasthan, Haryana & Uttar Pradesh",
    specs: [
      ["Purity", "100% De-Hulled Whole Barley"],
      ["Moisture", "< 10.5% Max"],
      ["Beta-Glucan", "> 4.8 g / 100g"],
      ["Dietary Fiber", "> 13.5%"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Wholesome, slow-stone-ground flour milled from clean, de-hulled Indian six-row barley. Naturally rich in heart-healthy soluble beta-glucan fiber, delivering cooling properties, low glycemic index, and sustained digestive balance.",
    highlights: ["High Soluble Beta-Glucan Fiber", "Cooling Digestive & Low GI Profile", "Traditional Diabetic Rotis & Baking"],
  },
  {
    id: "browntop-millet-flour",
    name: "Brown Top Millet Flour",
    localName: "Korale Ka Atta / Hari Kangni Flour",
    botanical: "Brachiaria ramosa",
    image: "/images/flours/browntop-millet-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Karnataka & Andhra Pradesh",
    specs: [
      ["Purity", "100% Native Browntop Millet"],
      ["Moisture", "< 9.5% Max"],
      ["Dietary Fiber", "> 12.5% (Highest Among Millets)"],
      ["Gluten Content", "0.0% Naturally Gluten-Free"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Rustic, nutrient-dense flour stone-ground from rare, unpolished brown top millet seeds. Contains the highest dietary fiber content (12.5%) of all millets, creating a potent prebiotic profile for gut health, toxin elimination, and glycemic regulation.",
    highlights: ["Record 12.5% Dietary Fiber", "Potent Prebiotic Gut Health Booster", "Zero Chemical Processing or Additives"],
  },
  {
    id: "proso-millet-flour",
    name: "Proso Millet Flour",
    localName: "Chena Ka Atta / Barri Flour",
    botanical: "Panicum miliaceum",
    image: "/images/flours/proso-millet-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Bihar, Uttar Pradesh & Karnataka",
    specs: [
      ["Purity", "100% Pure Unpolished Proso Millet"],
      ["Moisture", "< 10.0% Max"],
      ["Protein Content", "> 12.5% (High Leucine)"],
      ["Gluten Content", "0.0% Naturally Gluten-Free"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Delicate, pale-cream flour milled from de-hulled proso millet kernels. Boasts the highest protein concentration (12.5%) among minor millets, along with abundant niacin (Vitamin B3) and magnesium, making it an outstanding gluten-free flour for artisan breads and nutritious breakfast formulations.",
    highlights: ["Highest Millet Protein (12.5%)", "Rich in Niacin (Vitamin B3) & Zinc", "Smooth Silky Baking Consistency"],
  },
  {
    id: "finger-millet-ragi-flour",
    name: "Finger Millet / Ragi Flour",
    localName: "Ragi Ka Atta / Mandua Flour / Nachni Atta",
    botanical: "Eleusine coracana",
    image: "/images/flours/finger-millet-ragi-flour.jpg",
    hsCode: "1102.90.00",
    origin: "Karnataka, Uttarakhand & Tamil Nadu",
    specs: [
      ["Purity", "100% Whole Finger Millet"],
      ["Moisture", "< 10.0% Max"],
      ["Calcium Content", "> 344 mg / 100g (Highest of All Cereals)"],
      ["Gluten Content", "0.0% Naturally Gluten-Free"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Nutrient-packed ruby-brown flour milled from native unpolished finger millet (ragi). Naturally celebrated for containing the highest concentration of bioavailable plant calcium (over 344 mg/100g), natural iron, and polyphenols. Essential for diabetic wellness, weaning baby foods, and traditional ragi mudde, dosas, and rotis.",
    highlights: ["Highest Natural Plant Calcium (344mg)", "Bone Density & Diabetic Superfood", "100% Gluten-Free Ruby Whole Meal"],
  },
  {
    id: "gluten-free-flour-atta",
    name: "Gluten Free Flour / Gluten Free Atta Flour",
    localName: "Certified Gluten-Free Multi-Millet Atta",
    botanical: "Multi-Millet & Ancient Grain Blend",
    image: "/images/flours/gluten-free-flour-atta.jpg",
    hsCode: "1102.90.00",
    origin: "Madhya Pradesh, Rajasthan & Gujarat",
    specs: [
      ["Gluten Content", "0.0% (NABL Certified Allergen-Free)"],
      ["Moisture", "< 10.5% Max"],
      ["Composition", "Jowar + Ragi + Bajra + Amaranth + Quinoa + Chana"],
      ["Dietary Fiber", "> 11.5%"],
      ["Shelf Life", "12 Months Sealed"],
    ],
    description: "Premium 100% certified allergen-free composite flour meticulously crafted from stone-ground ancient grains: white sorghum (jowar), finger millet (ragi), bajra, ancient amaranth, and quinoa. Formulated to mimic traditional wheat elasticity naturally for soft, pliable rotis, parathas, and artisan gluten-free baking.",
    highlights: ["100% Certified Gluten-Free (0.0% Gluten)", "6-Grain Superfood Synergy", "Soft & Pliable Roti Elasticity"],
  },
];

// ============================================================
// SWEETENERS VARIETIES (Certified Indian Natural Sweeteners)
// ============================================================
export const SWEETENERS_VARIETIES = [
  {
    id: "jaggery-powder",
    name: "Jaggery Powder",
    localName: "Shakkar / Desi Gur Powder",
    botanical: "Saccharum officinarum",
    image: "/images/sweeteners/jaggery-powder.jpg",
    hsCode: "1701.14.10",
    origin: "Kolhapur (Maharashtra) & Muzaffarnagar (UP)",
    specs: [
      ["Purity", "100% Chemical-Free Artisanal"],
      ["Moisture", "< 4.0% Max"],
      ["Sucrose Content", "> 78.0%"],
      ["Mineral Content", "Rich in Natural Iron, Magnesium & Potassium"],
      ["Shelf Life", "18 Months"],
    ],
    description: "100% chemical-free, open-pan boiled artisanal jaggery ground to fine granulated powder. Completely free of sodium hydrosulphite or bleaching agents, rich in natural minerals and plant iron.",
    highlights: ["100% Chemical-Free & Hydros-Free", "Rich in Natural Plant Iron", "Instant Dissolving Granules"],
  },
  {
    id: "jaggery-cubes",
    name: "Jaggery Cubes",
    localName: "Desi Gur Cubes / Jaggery Blocks",
    botanical: "Saccharum officinarum",
    image: "/images/sweeteners/jaggery-cubes.jpg",
    hsCode: "1701.14.10",
    origin: "Kolhapur (Maharashtra) & Muzaffarnagar (UP)",
    specs: [
      ["Purity Standard", "100% Chemical-Free Artisanal"],
      ["Moisture", "< 5.0% Max"],
      ["Sucrose Content", "> 76.0%"],
      ["Mineral Content", "Rich in Iron, Calcium & Magnesium"],
      ["Format", "Uniform Solid Cut Cubes"],
      ["Shelf Life", "18 Months"],
    ],
    description: "Traditionally prepared from fresh, clarified sugarcane juice simmered in open pans and set into uniform solid cubes. Free of chemical clarifying agents, sulfur, or artificial food colorings, offering a rich caramel-molasses taste and wholesome minerals.",
    highlights: ["100% Natural & Chemical-Free", "Rich in Bioavailable Iron & Minerals", "Convenient Portion-Controlled Cubes", "Authentic Deep Caramel Taste"],
  },
  {
    id: "brown-sugar",
    name: "Brown Sugar",
    localName: "Raw Demerara / Brown Cane Sugar",
    botanical: "Saccharum officinarum",
    image: "/images/sweeteners/brown-sugar.jpg",
    hsCode: "1701.14.90",
    origin: "Maharashtra & Uttar Pradesh",
    specs: [
      ["Purity", "99.5% Natural Cane Sugar"],
      ["Moisture", "< 0.15% Max"],
      ["Color Rating", "ICUMSA 600 – 1200"],
      ["Molasses Content", "Natural Cane Molasses Coating"],
      ["Shelf Life", "24 Months"],
    ],
    description: "Unrefined sparkling golden-brown crystalline cane sugar retaining natural cane molasses. Imparts deep caramel notes and moist texture, ideal for gourmet bakery, beverage syrups, and specialty coffee blending.",
    highlights: ["Natural Cane Molasses Coating", "Rich Caramel Flavor Notes", "Free-Flowing Sparkling Crystals"],
  },
];

// ============================================================
// DALIYA / PORRIDGE VARIETIES (Certified Indian Export Daliya)
// ============================================================
export const DALIYA_VARIETIES = [
  {
    id: "wheat-daliya",
    name: "Wheat Daliya",
    localName: "Durum Lapsi Rawa / Broken Wheat",
    botanical: "Triticum durum",
    image: "/images/daliya/wheat-daliya.jpg",
    hsCode: "1103.11.00",
    origin: "Madhya Pradesh & Uttar Pradesh",
    specs: [
      ["Purity", "100% Golden Durum Wheat"],
      ["Moisture", "< 10.5% Max"],
      ["Granule Size", "Uniform Calibration (1.5 - 2.0 mm)"],
      ["Dietary Fiber", "> 11.5%"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Coarsely ground from select hard golden durum wheat. Pre-sifted for uniform grit calibration, delivering consistent cooking texture for sweet lapsi desserts, upma, and savory khichdi.",
    highlights: ["100% Golden Durum Wheat", "Uniform Grit Sizing (1.5–2.0mm)", "Non-Sticky Fluffy Cook"],
  },
  {
    id: "bajra-daliya",
    name: "Bajra Daliya / Pearl Millet Porridge",
    localName: "Bajre Ka Daliya / Pearl Millet Porridge",
    botanical: "Pennisetum glaucum",
    image: "/images/daliya/bajra-daliya.jpg",
    hsCode: "1103.19.00",
    origin: "Rajasthan, Gujarat & Haryana",
    specs: [
      ["Purity", "100% Unpolished Bajra Grits"],
      ["Moisture", "< 10.0% Max"],
      ["Iron Content", "High Bioavailable Iron"],
      ["Gluten Content", "0.0% Naturally Gluten-Free"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Coarsely crushed unpolished desert pearl millet. Renowned for its rich concentration of plant iron, phosphorus, and slow-burning complex carbs, delivering warming vitality and digestive wellness.",
    highlights: ["High Bioavailable Plant Iron", "Slow Digesting Energy", "Naturally Gluten-Free Grain Base"],
  },
  {
    id: "barley-daliya",
    name: "Barley Daliya / Jau Daliya",
    localName: "Jau Ka Daliya / Cracked Barley",
    botanical: "Hordeum vulgare",
    image: "/images/daliya/barley-daliya.jpg",
    hsCode: "1103.19.00",
    origin: "Rajasthan, Uttar Pradesh & Haryana",
    specs: [
      ["Purity Standard", "100% De-Hulled Whole Barley"],
      ["Moisture", "< 10.5% Max"],
      ["Beta-Glucan", "> 4.5%"],
      ["Dietary Fiber", "> 14.0%"],
      ["Shelf Life", "12 Months"],
    ],
    description: "De-hulled, clean whole barley cracked into uniform cooking granules. Packed with heart-healthy soluble beta-glucan fiber, widely recommended for glycemic management and cooling summer diets.",
    highlights: ["High Soluble Beta-Glucan", "Supports Blood Sugar Balance", "Light & Cooling on Digestion"],
  },
  {
    id: "multigrain-daliya",
    name: "Multi Grain Daliya",
    localName: "Navratna Multigrain Porridge",
    botanical: "Wheat + Moong + Millets Blend",
    image: "/images/daliya/multigrain-daliya.jpg",
    hsCode: "1103.19.00",
    origin: "Madhya Pradesh, Rajasthan & UP",
    specs: [
      ["Composition", "Durum Wheat + Green Moong + Brown Rice + Millets"],
      ["Moisture", "< 11.0% Max"],
      ["Protein", "> 13.0%"],
      ["Dietary Fiber", "> 12.0%"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Nutritionally balanced blend of broken durum wheat, green moong splits, brown rice grits, and roasted pearl millets. Provides complete protein, dietary fiber, and wholesome satiety for everyday nutrition.",
    highlights: ["Complete Plant Protein Profile", "Cracked Grains & Lentils Blend", "Wholesome Comfort Food"],
  },
  {
    id: "gluten-free-daliya",
    name: "Gluten Free Daliya",
    localName: "Multigrain Gluten-Free Porridge",
    botanical: "Millet & Ancient Grain Blend",
    image: "/images/daliya/gluten-free-daliya.jpg",
    hsCode: "1103.19.00",
    origin: "Madhya Pradesh & Gujarat",
    specs: [
      ["Purity", "100% Gluten-Free Certified Base"],
      ["Moisture", "< 10.0% Max"],
      ["Gluten Content", "0.0% (NABL Certified)"],
      ["Digestibility", "Ultra High Bioavailability"],
      ["Shelf Life", "12 Months"],
    ],
    description: "100% certified gluten-free porridge blend crafted from cracked sorghum, foxtail millet, and roasted quinoa grits. Cooks into a rich, creamy, allergen-free breakfast porridge.",
    highlights: ["100% Certified Gluten-Free", "Rich Golden Grits Texture", "Hypoallergenic Superfood Blend"],
  },
  {
    id: "jowar-daliya",
    name: "Jowar Daliya / Sorghum Porridge",
    localName: "Jowar Ka Daliya / Cracked Sorghum Grits",
    botanical: "Sorghum bicolor",
    image: "/images/daliya/jowar-daliya.jpg",
    hsCode: "1103.19.00",
    origin: "Maharashtra, Karnataka & Madhya Pradesh",
    specs: [
      ["Purity Standard", "100% De-hulled Whole Sorghum Grits"],
      ["Moisture", "< 10.0% Max"],
      ["Gluten Content", "0.0% Naturally Gluten-Free"],
      ["Dietary Fiber", "> 10.0%"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Coarsely cracked from premium de-hulled white sorghum. Naturally gluten-free and packed with polyphenols, resistant starch, and essential minerals, creating a gentle, diabetic-friendly breakfast porridge.",
    highlights: ["Antioxidant-Rich Polyphenols", "Naturally Gluten-Free Grain", "Low Glycemic Slow Energy", "Gentle on Digestion"],
  },
  {
    id: "maize-daliya",
    name: "Maize Daliya / Corn Porridge",
    localName: "Makka Daliya / Golden Corn Grits",
    botanical: "Zea mays",
    image: "/images/daliya/maize-daliya.jpg",
    hsCode: "1103.13.00",
    origin: "Madhya Pradesh, Karnataka & Rajasthan",
    specs: [
      ["Purity Standard", "100% Sun-Ripened Yellow Corn Grits"],
      ["Moisture", "< 11.0% Max"],
      ["Carotenoids", "High Natural Beta-Carotene"],
      ["Fat / Oil Content", "< 1.5% Degermed Grits"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Coarsely milled from degermed golden sun-ripened corn kernels. Provides vibrant natural golden color, wholesome carotenoid antioxidants, and a light, crumbly texture ideal for breakfast polenta, porridge, and upma.",
    highlights: ["Natural Carotenoids & Energy", "Vibrant Golden Grain Texture", "Naturally Gluten-Free Base", "Versatile Breakfast Polenta / Upma"],
  },
  {
    id: "mix-millets-daliya",
    name: "Mix Millets Daliya / Multi Millet Porridge",
    localName: "Mixed Millets Daliya / Navratna Millet Porridge",
    botanical: "Sorghum + Pearl + Foxtail + Ragi + Kodo Blend",
    image: "/images/daliya/mix-millets-daliya.jpg",
    hsCode: "1103.19.00",
    origin: "Rajasthan, Karnataka, MP & Uttarakhand",
    specs: [
      ["Composition", "Cracked Blend of 5 Heritage Indian Millets"],
      ["Moisture", "< 10.0% Max"],
      ["Gluten Content", "0.0% Certified Gluten-Free"],
      ["Mineral Density", "High Bioavailable Iron & Calcium"],
      ["Shelf Life", "12 Months"],
    ],
    description: "Artisanal superfood porridge blending coarsely cracked Sorghum, Pearl Millet (Bajra), Finger Millet (Ragi), Foxtail, and Kodo millets. Combines multi-millet micronutrients into one balanced, highly satisfying daily breakfast.",
    highlights: ["Powerhouse 5-Millet Synergy", "High Natural Calcium & Iron", "100% Certified Gluten-Free", "Sustained Satiety & Wellness"],
  },
];

// ============================================================
// VARIETY ADVANTAGE & HIGHLIGHT MAPPERS (Sortex-Free, HS-Free)
// ============================================================
export const getVarietyAdvantage = (v) => {
  if (v.advantage) return v.advantage;
  const map = {
    // Millets
    "pearl-millet": "High Iron & Zinc Supercrop",
    "finger-millet": "Highest Natural Calcium",
    "foxtail-millet": "Diabetic-Friendly Low GI",
    "little-millet": "Essential Healthy PUFA Fats",
    "kodo-millet": "Cardiovascular Polyphenols",
    "sorghum-millet": "Antioxidant-Rich Gluten-Free",
    "proso-millet": "Complete Plant Protein (12.5%)",
    "browntop-millet": "Maximum Dietary Fiber (12.5%)",
    "barnyard-millet": "Lowest Calorie & Highest Iron",
    "buckwheat-millet": "Complete Protein & Bioactive Rutin",

    // Pulses
    "pigeon-pea-split-toor-dal": "High Plant Protein (>22%)",
    "cowpea-white-whole-lobiya": "Soluble Fiber & Folate Rich",
    "green-gram-split-dehusked-moong-dal": "Lightest & Easiest to Digest",
    "green-gram-whole-moong-whole": "High Sprouting Supercrop (>95%)",
    "green-gram-split-moong-dal-chhilka": "Fiber-Rich Husk & Gentle Digesting",
    "red-lentil-split-masoor-dal-split": "Fast Cooking & High Iron",
    "brown-lentil-masoor-whole": "Earthy Texture & High Satiety",
    "red-lentil-whole-masoor-dal": "Mineral Dense & Slow Burning",
    "red-lentil-whole-football-masoor": "Mineral Dense & Slow Burning",
    "bengal-gram-whole-kala-chana": "Low Glycemic Sustained Energy",
    "chickpea-bengal-gram-kala-chana": "Low Glycemic Sustained Energy",
    "bengal-gram-split-chana-dal": "Low GI & Zinc-Rich Golden Dal",
    "chickpea-kabuli-chana": "Extra Bold Export Calibration & High Protein",
    "black-gram-whole-urad-whole": "High Frothing & Fermentation Star",
    "black-gram-split-urad-dal-split": "High Fiber Split & Iron",
    "black-gram-split-urad-split-chilka": "High Fiber Split & Iron",
    "black-gram-split-dehusked-urad-white-dal": "Ultra-Pure Culinary Protein",
    "black-gram-split-dehusked-urad-dhuli": "Ultra-Pure Culinary Protein",
    "cowpea-red-whole-red-lobiya": "Anthocyanin Rich & Firm Texture",
    "kidney-bean-chitra-rajma-chitra": "Himalayan Buttery & High Folate",
    "kidney-bean-jammu-rajma-jammu": "GI-Heritage Gourmet & Naturally Sweet",
    "red-kidney-bean-rajma-red": "Classic Large Grain & Rich Protein",
    "soybean-whole": "Ultra-High Protein (>38%)",
    "soybean-whole-yellow": "Ultra-High Protein (>38%)",

    // Grains
    "maize-corn-makka": "Natural Carotenoids & Energy",
    "amaranth-rajgira": "Ancient Grain & Complete Lysine",
    "barley-jau": "Heart-Healthy Soluble Beta-Glucan",
    "1121-xxl-basmati-rice": "1121 XXL Aged Aromatic Slender Grain",
    "sharbati-wheat": "Golden Sheen & High Natural Protein",
    "wheat-whole": "High Protein & Superior Gluten Strength",
    "quinoa": "Complete 9-Amino Acid Superseed",
    "oats-dehulled": "Whole Groat Bran & High Beta-Glucan",
    "oats-rolled": "Cardiovascular Beta-Glucan & Satiety",
    "oats-flakes": "Cardiovascular Beta-Glucan & Satiety",
    "white-poha-beaten-rice": "Ultra-Light & Easy Digesting Iron Carrier",

    // Sweeteners
    "jaggery-powder": "100% Chemical-Free & Mineral Rich",
    "jaggery-cubes": "Traditional Solid Gur & Iron Rich",
    "brown-sugar": "Natural Molasses & Unrefined Crystals",

    // Flours
    "amaranth-rajgira-flour": "Ancient Protein Powerhouse",
    "besan-bengal-gram-flour": "100% Pure Chana Dal (Zero Additives)",
    "barnyard-millet-flour": "Lowest Calorie & High Iron Density",
    "buckwheat-kuttu-flour": "Bioactive Rutin & Naturally Gluten-Free",
    "foxtail-millet-flour": "Low Glycemic Index Everyday Flour",
    "oats-flour": "Heart-Healthy Soluble Beta-Glucan",
    "kodo-millet-flour": "Polyphenol & Prebiotic Rich",
    "maida-refined-wheat-flour": "Superfine Bakery & Confectionery Grade",
    "maize-corn-makka-flour": "Golden Sun-Ripened Sweet Corn",
    "multigrain-flour-atta": "Powerhouse 6-Grain Balanced Protein",
    "whole-wheat-chakki-atta": "Stone-Ground with Intact Natural Bran",
    "pearl-millet-bajra-flour": "Iron-Rich Gluten-Free Superflour",
    "quinoa-flour": "Complete Plant Amino Acid Profile",
    "semolina-suji-flour": "Durum Wheat High Granular Stability",
    "sorghum-millet-jowar-flour": "Gentle Digesting Diabetic-Friendly",
    "little-millet-flour": "Essential Healthy PUFA Fats & Iron Rich",
    "rice-chawal-flour": "Hypoallergenic & Ultra-Light Digestibility",
    "barley-jau-flour": "Heart-Healthy Beta-Glucan & Cooling Metabolism",
    "browntop-millet-flour": "Maximum Dietary Fiber (12.5%) & Prebiotic Power",
    "proso-millet-flour": "Highest Plant Protein (12.5%) & Complete Nutrition",
    "finger-millet-ragi-flour": "Highest Natural Calcium & Bone Density",
    "gluten-free-flour-atta": "100% Certified Gluten-Free & Multi-Millet Synergy",

    // Daliya
    "wheat-daliya": "Slow-Roasted High Satiety Durum",
    "bajra-daliya": "Iron & Calcium Rich Winter Fuel",
    "jowar-daliya": "Antioxidant Polyphenols & Low GI",
    "barley-daliya": "Cardiovascular Soluble Beta-Glucan",
    "maize-daliya": "Natural Carotenoids & Energy",
    "multigrain-daliya": "Synergy of 5 Heritage Grains",
    "mix-millets-daliya": "Powerhouse 5-Millet Synergy",
    "gluten-free-daliya": "100% Certified Multi-Millet Allergen-Free",
  };
  return map[v.id] || "Export Grade Premium";
};

export const getVarietyNote = (v) => {
  const cleanHighlights = (v.highlights || []).filter(
    (h) => !h.toLowerCase().includes("sortex")
  );
  return cleanHighlights.join(" · ");
};

// ============================================================
// PRODUCTS — restructured with brand range + category
// ============================================================
export const PRODUCTS = [
  // ─── HARVESTGATE SELECT ───────────────────────────────────
  {
    slug: "foxnuts",
    index: "01",
    range: "SELECT",
    category: "foxnuts",
    categoryLabel: "Premium Foxnuts / Makhana",
    name: "Premium Foxnuts",
    subtitle: "Popped Lotus Seeds / Phool Makhana",
    tagline:
      "At Harvestgate Overseas, we bring you the finest grade of handpicked premium Indian foxnuts (makhana), cultivated in the most fertile regions of Bihar and processed with utmost care to preserve their natural taste, texture, and nutrition.",
    accent: "#F7F4EB",
    accentLight: "#8A7343",
    accentName: "Cream / Ivory",
    origin: "Mithila Region, Bihar, India",
    hsCode: "0802.99.00",
    season: "August — November",
    image: IMG.foxnuts,
    intro:
      "At Harvestgate Overseas, we bring you the finest grade of handpicked premium Indian foxnuts (makhana), cultivated in the most fertile regions of Bihar and processed with utmost care to preserve their natural taste, texture, and nutrition. Our handpicked foxnuts are crisp, clean, and ready for a wide range of uses — from healthy snacking to gourmet cooking.",
    description:
      "Don’t just watch the demand grow — own it. Import our handpicked premium organic foxnuts into your country, put them under your own brand, and become the go-to supplier in your market. The demand is rising, the margins are attractive, and the time to act is now. Secure your first shipment today and start building your profitable foxnut empire.",
    suttaExplanation: {
      title: "What is “Sutta” in Foxnuts (Makhana)?",
      text: "In foxnut grading in India, 1 sutta ≈ 3.2 mm in diameter. The “sutta” count reflects the approximate size of each popped kernel — higher counts mean larger, more premium foxnuts prized for their look, texture, and crunch.",
      ratio: "1 Sutta ≈ 3.2 mm Diameter",
    },
    whyChoose: [
      {
        title: "Top Quality Premium",
        desc: "Handpicked, crisp, and naturally nutritious with high puff volume.",
      },
      {
        title: "Ethically Sourced",
        desc: "Direct from trusted Indian farmers in Bihar's fertile wetland belt.",
      },
      {
        title: "Testing on Request",
        desc: "Paid testing can be arranged at FSSAI-approved & NABL-accredited labs.",
      },
      {
        title: "Customizable Packaging",
        desc: "Generally packed in HDPE bags of 10 kg or 5 kg for bulk export, but it can be customised as per buyer standards.",
      },
      {
        title: "Bulk Supply Experts",
        desc: "Reliable, consistent global container shipments.",
      },
      {
        title: "Profit Potential",
        desc: "Import, rebrand, and grow your foxnut business.",
      },
    ],
    grades: [
      {
        name: "Grade A : 6+ Sutta HP",
        image: "/images/foxnuts/6-sutta.jpg",
        spec: "6+ Sutta HP (≈ 19.2 mm+)",
        shortDesc: "Extra-large, handpicked superior-grade foxnuts with a rich, glossy appearance and unmatched crunch.",
        note: "Exclusive selection of handpicked 6+ sutta and larger sizes; extra-large, superior-grade handpicked foxnuts with a rich, glossy appearance and exceptional texture that stands out on any shelf. Delivers unmatched crunch, premium presentation, and handpicked consistency — making it a preferred choice for high-end retail, gourmet packaging, and export markets seeking top-tier quality.",
      },
      {
        name: "Grade B : 5+ Sutta HP",
        image: "/images/foxnuts/5-sutta.png",
        spec: "5+ Sutta HP (≈ 16.0 – 19.2 mm)",
        shortDesc: "Handpicked blend of 5+ sutta HP; uniform, attractive, and versatile — perfect for snacks and rebranding.",
        note: "Premium blend of handpicked 5+ sutta HP; uniform, attractive, and versatile — perfect for snacks, gourmet blends, and rebranding. Handpicked to offer consistent quality, excellent puff, and a rich texture that enhances product appeal across diverse markets.",
      },
      {
        name: "Grade C : 4+ Sutta HP",
        image: "/images/foxnuts/4-sutta.png",
        spec: "4+ Sutta HP (≈ 12.8 – 16.0 mm)",
        shortDesc: "Handpicked assortment of 4+ sutta HP delivering consistent quality and excellent puff.",
        note: "A premium assortment of handpicked 4+ sutta HP foxnuts with a uniform and appealing appearance. Carefully handpicked for snacks, gourmet mixes, and rebranding, delivering consistent quality, excellent puff, and a rich texture that enhances overall product attractiveness across varied markets.",
      },
    ],
    variationDisclaimer:
      "Images shown are for reference purposes; actual product may exhibit natural variations in size, color, and texture depending on the harvest — a hallmark of its authenticity and freshness.",
    howToOrder:
      "To start your order, simply fill out our official enquiry form mentioned below or email us at contact@harvestgateoverseas.com with your requirements. Our sales team will connect with you to discuss details and share Ex-Works pricing (valid for 3 days). Once specifications, shipping, and final pricing are agreed upon, we’ll issue a Proforma Invoice (PI). The order is confirmed upon receipt of the advance payment as per our payment terms.",
    orderTerms: {
      moq: "As per buyer's demand",
      prepTime: "Minimum 5 weeks (may vary based on quantity, shipping terms, and harvest availability)",
      samples: "Minimum 1 kg, chargeable with prepaid shipping costs",
      validity: "Ex-Works rate valid for upcoming 3 days only",
    },
    specs: [
      ["Moisture Content", "< 8.0 % Max"],
      ["Purity Level", "99.8 % Cleaned"],
      ["Calibration Unit", "1 Sutta ≈ 3.2 mm Diameter"],
      ["Broken / Black Spot", "< 1.0 % Max"],
      ["Export Packaging", "from 250 gms to 10 kg Export HDPE Bags (Customizable)"],
      ["Shelf Life", "18 — 24 Months Sealed"],
    ],
    moq: "As per buyer's demand",
    incoterms: ["FOB Kolkata", "FOB Nhava Sheva", "CIF", "CFR", "EXW Facility", "Air Freight"],
    markets: ["United States", "United Arab Emirates", "United Kingdom", "Australia", "Canada", "European Union"],
  },

  // ─── HARVESTGATE NATURALS ─────────────────────────────────
  {
    slug: "millets",
    index: "02",
    range: "NATURALS",
    category: "millets",
    categoryLabel: "Millets",
    name: "Authentic Indian Millets",
    subtitle: "• Pearl • Finger • Foxtail • Little • Kodo • Sorghum • Proso • Brown Top • Barnyard • Buckwheat",
    tagline: "Wholesome Indian millets, naturally cultivated and carefully processed to deliver authentic taste, rich nutrition, and the goodness of traditional grains.",
    accent: "#8A9A86",
    accentLight: "#4C5A48",
    accentName: "Olive / Sage",
    origin: "Rajasthan, Karnataka, MP & Uttarakhand, India",
    hsCode: "1008.21.00 / 1008.29.00 / 1007.90.00 / 1008.10.00",
    season: "Year-Round Export Availability",
    image: IMG.millets,
    intro:
      "At HarvestGate Overseas, we bring you the finest grade of premium Indian millets — Sorghum (Jowar), Pearl Millet (Bajra), Finger Millet (Ragi), Foxtail Millet, Barnyard Millet, Kodo Millet, Proso Millet, Little Millet, Brown Top Millet, and Buckwheat (Kuttu). Cultivated in fertile, chemical-free drylands and naturally cleaned to preserve natural nutrition, color, and texture.",
    description:
      "We aggregate directly from contracted farmer producer groups across 5 states. Every lot undergoes multi-stage cleaning, gravity separation, de-stoning, and magnetic inspection. Consignments are backed by full NABL-accredited and FSSAI-approved laboratory reports covering pesticide residues, heavy metals, and nutritional profiles.",
    grades: MILLET_VARIETIES.map((v) => ({
      name: `${v.name} (${v.localName})`,
      spec: getVarietyAdvantage(v),
      note: getVarietyNote(v),
    })),
    varieties: MILLET_VARIETIES,
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture Limit", "< 10.0% Max"],
      ["Foreign Matter", "< 0.2% - 0.3% Max"],
      ["Pesticide Residues", "Zero / Below Detection Limit (NABL Certified)"],
      ["Shelf Life", "24 Months Sealed"],
      ["Packaging Formats", "250g - 1kg Vacuum Pouches / 25kg - 50kg HDPE Woven Bags"],
    ],
    moq: "1 x 20ft FCL — 24 to 25 MT (Consolidated multi-millet containers available)",
    incoterms: ["FOB Mundra", "FOB Nhava Sheva", "CIF", "CFR", "EXW"],
    markets: ["Germany", "United States", "United Kingdom", "Saudi Arabia", "UAE", "Singapore", "Australia", "Japan"],
  },
  {
    slug: "sweeteners",
    index: "03",
    range: "NATURALS",
    category: "sweeteners",
    categoryLabel: "Sweeteners",
    name: "Natural Indian Sweeteners",
    subtitle: "• Brown Sugar • Jaggery Cube • Jaggery Powder",
    tagline: "Pure, traditionally crafted jaggery and sugars, made with care to bring rich flavour, natural sweetness, and authentic Indian goodness to every bite.",
    accent: "#D99B26",
    accentLight: "#8F6208",
    accentName: "Deep Gold / Ochre",
    origin: "Maharashtra & Uttar Pradesh, India",
    hsCode: "1701.14.90 / 1701.14.10",
    season: "Year-round export availability",
    image: "/images/sweeteners/jaggery-powder.jpg",
    intro:
      "At HarvestGate Overseas, our export sweeteners programme delivers pure, chemical-free and unbleached cane sweeteners: Artisanal Granulated Jaggery Powder (Gur Shakkar), Traditional Solid Jaggery Cubes (Desi Gur Cubes), and Sparkling Golden Raw Brown Cane Sugar.",
    description:
      "HarvestGate contracts directly with artisanal open-pan gur producers in Kolhapur and Western UP. Zero sodium hydrosulphite, zero chemical bleaches, and 100% trace mineral retention with NABL and FSSAI export certificates.",
    grades: SWEETENERS_VARIETIES.map((v) => ({
      name: `${v.name} (${v.localName.split("/")[0].trim()})`,
      spec: getVarietyAdvantage(v),
      note: getVarietyNote(v),
    })),
    varieties: SWEETENERS_VARIETIES,
    specs: [
      ["Purity Standard", "100% Natural Cane / Hydros-Free"],
      ["Moisture Limit", "< 0.15% - 4.0% Max"],
      ["Sucrose / Polarization", "> 78.0% - 99.5%"],
      ["Chemical Additives", "Zero / NABL Certified"],
      ["Shelf life", "18 - 24 months"],
      ["Packaging", "500g / 1kg retail pouches & 25kg / 50kg food-grade master bags"],
    ],
    moq: "1 x 20ft FCL — 25 MT",
    incoterms: ["FOB Nhava Sheva", "FOB Mundra", "CIF", "CFR"],
    markets: ["United States", "Middle East", "Kenya", "New Zealand", "European Union"],
  },
  {
    slug: "pulses",
    index: "04",
    range: "NATURALS",
    category: "pulses",
    categoryLabel: "Pulses",
    name: "Premium Indian Pulses",
    subtitle: "• Chickpea • Pigeon Pea • Moong Split • Moong Whole • Moong Dehusked • Chana Whole • Chana Split • Brown Lentil • Red Lentil Whole • Red Lentil Split • Black Gram Whole • Black Gram Split • Black Gram Dehusked • White Cowpea • Red Cowpea • Red Kidney Bean • Jammu Kidney Bean • Chitra Kidney Bean • Soybean",
    tagline: "Premium Indian pulses, naturally cultivated and carefully selected for their freshness, hearty taste, and wholesome nutritional value.",
    accent: "#C26D45",
    accentLight: "#8C3B14",
    accentName: "Terracotta / Amber Pulse",
    origin: "Madhya Pradesh, Maharashtra, Rajasthan & UP, India",
    hsCode: "0713.60.00 / 0713.40.00 / 0713.20.00 / 0713.31.00 / 1201.90.00",
    season: "Year-round availability",
    image: "/images/pulses/pigeon-pea-split-toor-dal.jpg",
    intro:
      "At HarvestGate Overseas, we export 19 premium grades of machine-cleaned Indian pulses — Kabuli Chickpeas, Pigeon Peas (Toor/Arhar Dal), Chana Dal, Bengal Gram (Kala Chana), White & Red Cowpeas (Lobiya), Green Gram (Moong Whole, Split Chilka & Dhuli), Red & Brown Lentils (Masoor Split, Whole & Brown), Red, Jammu & Chitra Kidney Beans (Rajma), Black Gram (Urad Whole, Chilka & Dhuli), and Whole Soybeans.",
    description:
      "Sourced directly from primary agricultural mandis across Madhya Pradesh, Maharashtra, Rajasthan, Karnataka, and Andhra Pradesh. Each batch undergoes optical grading, de-stoning, and stringent moisture testing, backed by full FSSAI and APEDA export certifications.",
    grades: PULSES_VARIETIES.map((v) => ({
      name: `${v.name} (${v.localName.split("/")[0].trim()})`,
      spec: getVarietyAdvantage(v),
      note: getVarietyNote(v),
    })),
    varieties: PULSES_VARIETIES,
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture Limit", "< 10.0% - 10.5% Max"],
      ["Foreign Matter", "< 0.2% - 0.3% Max"],
      ["Broken Grains", "< 1.0% Max"],
      ["Shelf Life", "24 Months Sealed"],
      ["Packaging Formats", "500g - 1kg Vacuum Pouches / 25kg - 50kg PP & Jute Bags"],
    ],
    moq: "1 x 20ft FCL — 24 MT (Consolidated multi-pulse containers available)",
    incoterms: ["FOB Nhava Sheva", "FOB Mundra", "CIF", "CFR", "EXW"],
    markets: ["United Kingdom", "Canada", "United States", "UAE", "Singapore", "Australia", "Saudi Arabia"],
  },
  {
    slug: "grains",
    index: "05",
    range: "NATURALS",
    category: "grains",
    categoryLabel: "Grains",
    name: "Wholesome Indian Grains",
    subtitle: "• Wheat • Barley • Maize • Amaranth • Buckwheat • Quinoa • Oats • Oat Flakes • White Poha",
    tagline: "Quality Indian grains, thoughtfully cultivated and carefully processed to preserve their natural goodness, distinctive flavour, and wholesome character.",
    accent: "#6B8E63",
    accentLight: "#3A5C33",
    accentName: "Paddy Green / Field Sage",
    origin: "Punjab, Haryana, MP, Gujarat & Rajasthan, India",
    hsCode: "1005.90.00 / 1008.90.00 / 1003.90.00 / 1006.30.20 / 1001.99.10",
    season: "Year-round availability",
    image: "/images/grains/maize-corn-makka.jpg",
    intro:
      "At HarvestGate Overseas, our export grains and cereals programme delivers 10 prime harvests: Indian Yellow Maize (Corn), Ancient Amaranth (Rajgira), Golden Barley (Jau), 1121 XXL Aged Basmati Rice, Sharbati Milling Wheat, Whole Milling Wheat, White Royal Quinoa, Dehulled Whole Oat Groats, Rolled Oat Flakes, and Traditional White Poha (Flattened Rice).",
    description:
      "Directly sourced from trusted agrarian clusters and modernized grain silos across Punjab, Haryana, Madhya Pradesh, Gujarat, and Rajasthan. Consignments undergo stringent optical sorting, moisture monitoring, and full APEDA/FSSAI export certifications.",
    grades: GRAINS_VARIETIES.map((v) => ({
      name: `${v.name} (${v.localName.split("/")[0].trim()})`,
      spec: getVarietyAdvantage(v),
      note: getVarietyNote(v),
    })),
    varieties: GRAINS_VARIETIES,
    specs: [
      ["Purity Standard", "100% Natural Machine Cleaned"],
      ["Moisture Limit", "< 10.0% - 12.0% Max"],
      ["Damaged / Discolored", "< 0.5% Max"],
      ["Pesticide Residues", "Zero / NABL Certified"],
      ["Shelf life", "24 months"],
      ["Packaging", "10 kg / 25 kg / 50 kg BOPP, PP & Jute bags"],
    ],
    moq: "1 x 20ft FCL — 25 MT",
    incoterms: ["FOB Mundra", "FOB Kandla", "FOB Nhava Sheva", "CIF"],
    markets: ["Saudi Arabia", "Iraq", "Kuwait", "United Kingdom", "United States", "Yemen"],
  },
  {
    slug: "flours",
    index: "06",
    range: "NATURALS",
    category: "flours",
    categoryLabel: "Flours",
    name: "Traditional Indian Flours",
    subtitle: "• Multigrain • Gluten-Free • Whole Wheat • Wheat • Pearl • Finger • Foxtail • Little • Kodo • Sorghum • Proso • Brown Top • Barnyard • Barley • Maize • Amaranth • Buckwheat • Quinoa • Oats • Rice • Besan • Semolina • Maida",
    tagline: "Finely milled Indian flours made from carefully selected grains and millets, delivering authentic texture, freshness, and versatile everyday goodness.",
    accent: "#D4A96A",
    accentLight: "#8C6030",
    accentName: "Warm Wheat / Biscuit",
    origin: "Uttar Pradesh, Rajasthan, Gujarat, Maharashtra & MP, India",
    hsCode: "1101.00.00 / 1102.90.00 / 1106.10.00 / 1103.11.00",
    season: "Year-round availability",
    image: "/images/flours/multigrain-flour-atta.jpg",
    intro:
      "At HarvestGate Overseas, our wholesome flours programme features 22 stone-ground, 100% natural and unadulterated flours: Whole Wheat Chakki Atta, Multigrain Atta, Certified Gluten-Free Atta, Finger Millet (Ragi), Pearl Millet (Bajra), Sorghum (Jowar), Foxtail, Barnyard, Kodo, Little, Proso & Brown Top Millet Flours, Ancient Amaranth (Rajgira), Quinoa Flour, Pure Chana Besan, Buckwheat (Kuttu), Oats Flour, Barley (Jau) Flour, Rice Flour, Maize (Makka) Flour, Semolina (Suji), and Superfine Maida.",
    description:
      "Milled in modern HACCP-certified facilities with strict temperature control to preserve essential nutrients, natural aroma, and enzyme activity. Every batch undergoes rigorous quality checks for fineness, moisture, and zero additives.",
    grades: FLOURS_VARIETIES.map((v) => ({
      name: `${v.name} (${v.localName.split("/")[0].trim()})`,
      spec: getVarietyAdvantage(v),
      note: getVarietyNote(v),
    })),
    varieties: FLOURS_VARIETIES,
    specs: [
      ["Purity Standard", "100% Pure & Unadulterated"],
      ["Moisture Limit", "< 10.0% - 12.0% Max"],
      ["Ash Content", "< 1.0% Max"],
      ["Foreign Matter", "Zero / Sifted"],
      ["Shelf Life", "12 Months Sealed"],
      ["Packaging Formats", "500g - 1kg Vacuum Pouches / 25kg - 50kg Multi-wall Master Bags"],
    ],
    moq: "1 x 20ft FCL — 20 MT",
    incoterms: ["FOB Nhava Sheva", "FOB Mundra", "CIF", "CFR"],
    markets: ["United Kingdom", "United States", "UAE", "Canada", "Australia", "Germany"],
  },
  {
    slug: "daliya",
    index: "07",
    range: "NATURALS",
    category: "daliya",
    categoryLabel: "Porridge / Daliya",
    name: "Traditional Porridge & Daliya",
    subtitle: "• Wheat • Bajra • Jowar • Barley • Maize • Multigrain • Mixed Millet • Gluten-Free",
    tagline: "Wholesome Indian daliya (porridge) varities, carefully prepared for a nourishing, hearty, and naturally satisfying addition to everyday meals.",
    accent: "#B8860B",
    accentLight: "#7A5703",
    accentName: "Wheat Ochre / Golden Amber",
    origin: "Madhya Pradesh, Rajasthan & Uttar Pradesh, India",
    hsCode: "1103.11.00 / 1103.19.00",
    season: "Year-round availability",
    image: "/images/daliya/wheat-daliya.jpg",
    intro:
      "At HarvestGate Overseas, our roasted daliya and wholesome porridge programme delivers 8 certified grades: Wheat Daliya (Durum Lapsi Rawa), Bajra Daliya (Pearl Millet), Jowar Daliya (Sorghum Porridge), Barley Daliya (Jau), Maize Daliya (Corn Grits), Multi Grain Daliya, Mix Millets Daliya, and 100% Gluten Free Daliya.",
    description:
      "Manufactured in dust-free roller mill systems with de-stoners and optical sorters. Available in raw golden granules or slow-roasted formats that cook evenly without clumping, ideal for porridge, savory upma, and healthy meal formulations.",
    grades: DALIYA_VARIETIES.map((v) => ({
      name: `${v.name} (${v.localName.split("/")[0].trim()})`,
      spec: getVarietyAdvantage(v),
      note: getVarietyNote(v),
    })),
    varieties: DALIYA_VARIETIES,
    specs: [
      ["Purity Standard", "100% Natural De-stoned"],
      ["Moisture Limit", "< 10.0% - 10.5% Max"],
      ["Foreign Matter", "< 0.1% Max"],
      ["Dietary Fiber", "> 11.5% - 14.0%"],
      ["Shelf Life", "12 Months Sealed"],
      ["Packaging Formats", "500g - 1kg Retail Pouches / 25kg - 50kg Master Bags"],
    ],
    moq: "1 x 20ft FCL — 20 MT",
    incoterms: ["FOB Nhava Sheva", "FOB Mundra", "CIF", "CFR"],
    markets: ["United States", "United Kingdom", "Canada", "Australia", "UAE"],
  },
];

// ============================================================
// CATEGORIES CONFIG (for nav + filter tabs)
// ============================================================
export const NATURALS_CATEGORIES = [
  { id: "millets", label: "Millets", slug: "millets", description: "Pearl • Finger • Foxtail and more" },
  { id: "sweeteners", label: "Sweeteners", slug: "sweeteners", description: "Brown Sugar, Jaggery Cubes, Powder, etc." },
  { id: "pulses", label: "Pulses", slug: "pulses", description: "Chickpeas, Green Gram, Red Lentil and more" },
  { id: "grains", label: "Grains", slug: "grains", description: "Wheat, Barley, Quinoa and more" },
  { id: "flours", label: "Flours", slug: "flours", description: "Multigrain, Gluten-Free and more" },
  { id: "daliya", label: "Porridge / Daliya", slug: "daliya", description: "Wheat, Barley, Multigrain, Mix Millets and more" },
];

export const SELECT_CATEGORIES = [
  { id: "foxnuts", label: "Premium Foxnuts", slug: "foxnuts", description: "Popped Lotus Seeds / Phool Makhana" },
];

// ============================================================
// HELPERS
// ============================================================
export const getProduct = (slug) => PRODUCTS.find((p) => p.slug === slug);

export const getProductsByRange = (range) => PRODUCTS.filter((p) => p.range === range);

export const getProductsByCategory = (category) => PRODUCTS.filter((p) => p.category === category);

/* Accent readable against the current surface colour */
export const surfaceAccent = (product, theme) =>
  theme === "light" ? product.accentLight : product.accent;

/* Range badge config */
export const getRangeBadge = (rangeKey) => BRAND_RANGES[rangeKey];

// ============================================================
// MANIFESTO / STATS / CERTS (unchanged)
// ============================================================
export const MANIFESTO = [
  {
    number: "01",
    title: "Sustainably Sourced",
    icon: "sprout",
    theme: "gold",
    body: "We work with a reliable network of suppliers across India to source products that meet ethical standards, sustainability benchmarks, and international quality requirements.",
  },
  {
    number: "02",
    title: "Superior Product Quality",
    icon: "shield",
    theme: "green",
    body: "We deliver handpicked Foxnuts, wide range of Millets, Walnuts, and Chemical-Free Jaggery, with rigorous quality control. We also offer 3rd party or lab testing as per buyer requirements.",
  },
  {
    number: "03",
    title: "Custom Packaging Solutions",
    icon: "package",
    theme: "gold",
    body: "We provide flexible, hygienic, and customized packaging options suited to your brand and market needs, including private labelling services.",
  },
  {
    number: "04",
    title: "Certified & Compliant",
    icon: "award",
    theme: "green",
    body: "We are duly registered under GST, IEC, FSSAI, APEDA, and MSME, ensuring full compliance with statutory, food safety, and export regulations in line with Indian and international trade standards.",
  },
  {
    number: "05",
    title: "Reliable Customer Support",
    icon: "phone",
    theme: "gold",
    body: "From inquiry to shipment, we provide end-to-end service with responsive and personalized support.",
  },
  {
    number: "06",
    title: "Global Logistics & CHA Expertise",
    icon: "ship",
    theme: "green",
    body: "We ensure reliable, on-time worldwide shipping through efficient logistics and strong partnerships with experienced CHAs for smooth customs clearance and effective trade solutions.",
  },
];

export const CERTS = [
  "GSTIN: 09AAICH2946R1ZR",
  "Import Export Code : AAICH2946R",
  "APEDA Registered",
  "FSSAI Licensed",
  "MSME, Govt. of India Registered",
];
