...
async function sendSmtpEmail({ host, port, user, pass, fromEmail, fromName, toEmail, toName, subject, html, text, replyToEmail, replyToName }) {
  if (!user || !pass) {
  ...
  try {
    ...
    const info = await transporter.sendMail({
      from: `"${fromName || "HarvestGate Overseas"}" <${fromEmail || user}>`,
    ...
    console.log(`[SMTP] SUCCESS | Delivered to ${maskEmail(toEmail)} | messageId: ${info.messageId}`);
    return { ok: true, messageId: info.messageId, error: null };
  } catch (err) {
    console.error(`[SMTP] ERROR | Delivery failed to ${maskEmail(toEmail)}:`, err.message);
...
export default async function handler(req, res) {
  ...
  const adminEmail = process.env.MAIL_ADMIN_EMAIL || process.env.ADMIN_EMAIL || "admin@harvestgateoverseas.com";
  ...
  let buyerResult = { ok: false, error: "No email service configured" };
  let adminResult = { ok: false, error: "No email service configured" };

  // 1. Try Brevo HTTPS REST API first if apiKey configured
  const hasBrevo = apiKey && !apiKey.startsWith("your_");
  if (hasBrevo) {
    [adminResult, buyerResult] = await Promise.all([
      sendBrevoEmail({
  ...
  // 2. SMTP fallback if Brevo is not configured or if buyer email failed
  if (!buyerResult.ok && smtpUser && smtpPass) {
    console.log(`[Enquiry ${ref}] Falling back to SMTP relay...`);
    const [adminSmtp, buyerSmtp] = await Promise.all([
      sendSmtpEmail({
    ...
    if (buyerSmtp.ok) buyerResult = buyerSmtp;
    if (adminSmtp.ok) adminResult = adminSmtp;
  }

  console.log(`[Enquiry ${ref}] Result | Buyer Email: ${buyerResult.ok ? "DELIVERED (" + (buyerResult.messageId || "ok") + ")" : "FAILED (" + buyerResult.error + ")"} | Admin Email: ${adminResult.ok ? "DELIVERED (" + (adminResult.messageId || "ok") + ")" : "FAILED (" + adminResult.error + ")"}`);

  // Always return 200 OK so the visitor's enquiry is recorded and front-end receives response
  return res.status(200).json({
    success: true,
    ref,
    email_sent: buyerResult.ok,
    buyer_email: { sent: buyerResult.ok, message_id: buyerResult.messageId || null, error: buyerResult.error || null },
